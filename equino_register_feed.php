...
try {
    ...
    $concentradoQuery = "SELECT 
                            eh_concentrado_fecha_inicio,
                            eh_concentrado_fecha_fin,
                            eh_concentrado_racion,
                            eh_concentrado_costo,
                            eh_concentrado_tagid
                        FROM eh_concentrado 
                        WHERE eh_concentrado_fecha_inicio IS NOT NULL 
                        AND eh_concentrado_fecha_fin IS NOT NULL 
                        AND eh_concentrado_racion IS NOT NULL 
                        AND eh_concentrado_costo IS NOT NULL
                        ORDER BY eh_concentrado_fecha_inicio ASC";
    ...
    if (empty($concentradoData)) {
        ...
        $concentradoQuery = "SELECT 
                                eh_concentrado_fecha,
                                eh_concentrado_fecha as eh_concentrado_fecha_inicio,
                                eh_concentrado_fecha as eh_concentrado_fecha_fin,
                                eh_concentrado_racion,
                                eh_concentrado_costo,
                                eh_concentrado_tagid
                            FROM eh_concentrado 
                            WHERE eh_concentrado_fecha IS NOT NULL 
                            AND eh_concentrado_racion IS NOT NULL 
                            AND eh_concentrado_costo IS NOT NULL
                            ORDER BY eh_concentrado_fecha ASC";
    ...
    foreach ($concentradoData as $concentrado) {
        $startDate = new DateTime($concentrado['eh_concentrado_fecha_inicio']);
        $endDate = new DateTime($concentrado['eh_concentrado_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $concentrado['eh_concentrado_racion'] * $concentrado['eh_concentrado_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing concentrado: Start=" . $concentrado['eh_concentrado_fecha_inicio'] . 
                 ", End=" . $concentrado['eh_concentrado_fecha_fin'] . 
                 ", Days=" . $totalDays . 
...
<title>equino - Registro de Alimento Concentrado</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_alimento" name="alimento" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_alimentos = "SELECT DISTINCT ec_concentrado_nombre FROM ec_concentrado ORDER BY ec_concentrado_nombre ASC";
                                        ...
                                        foreach ($alimentos as $alimento_row) {
                                            echo '<option value="' . htmlspecialchars($alimento_row['ec_concentrado_nombre']) . '">' . htmlspecialchars($alimento_row['ec_concentrado_nombre']) . '</option>';
                                        }
                                    } catch (PDOException $e) {
  ...
  <!-- DataTable for eh_concentrado records -->
              ...
              <tbody>
                  ...
                  try {
                      // Query to get all Animals and ALL their concentrado records (if any)
                        $concentradoQuery = "
                            SELECT
                                v.tagid AS equino_tagid,
                                v.nombre AS animal_nombre,
                                v.etapa AS equino_etapa,
                                c.id AS concentrado_id,         -- Will be NULL for animals with no concentrado records
                                c.eh_concentrado_fecha_inicio,
                                c.eh_concentrado_fecha_fin,
                                c.eh_concentrado_tagid,         -- Matches equino_tagid if concentrado exists
                                c.eh_concentrado_etapa,
                                c.eh_concentrado_producto,
                                c.eh_concentrado_racion,
                                c.eh_concentrado_costo,
                                -- Calculate total_value only if c.id is not null
                                CASE WHEN c.id IS NOT NULL THEN CAST((c.eh_concentrado_racion * c.eh_concentrado_costo) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                equino v
                            LEFT JOIN
                                eh_concentrado c ON v.tagid = c.eh_concentrado_tagid -- Join ALL matching concentrado records
                            ORDER BY
                                ...
                                c.eh_concentrado_fecha_inicio DESC";
                      ...
                      if (empty($concentradoData)) {
                          ...
                          foreach ($concentradoData as $row) {
                              ...
                              $concentradoFecha = $row['eh_concentrado_fecha_inicio'] ?? null;
                              $concentradoFechaInicio = $row['eh_concentrado_fecha_inicio'] ?? null;
                              $concentradoFechaFin = $row['eh_concentrado_fecha_fin'] ?? null;
                              ...
                              echo '        <button class="btn btn-success btn-sm" 
                                              ...
                                              data-tagid-prefill="'.htmlspecialchars($row['equino_tagid'] ?? '').'" 
                              ...
                              if ($hasConcentrado) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-concentrado" 
                                                  ...
                                                  data-tagid="'.htmlspecialchars($row['equino_tagid'] ?? '').'" 
                                                  data-etapa="'.htmlspecialchars($row['equino_etapa'] ?? '').'" 
                                                  ...
                                                  data-debug="DB: '.htmlspecialchars($row['eh_concentrado_producto'] ?? 'NULL').'">
                              ...
                              echo "<td>" . htmlspecialchars($row['equino_tagid'] ?? 'N/A') . "</td>";
                              // Column 5: Etapa (or N/A)
                              echo "<td>" . htmlspecialchars($row['equino_etapa'] ?? 'N/A') . "</td>";
                              // Column 6: Producto (or N/A)
                              echo "<td>" . ($hasConcentrado ? htmlspecialchars($row['eh_concentrado_producto'] ?? '') : 'N/A') . "</td>";
                              // Column 7: Racion (or N/A)
                              echo "<td>" . ($hasConcentrado ? htmlspecialchars($row['eh_concentrado_racion'] ?? '') : 'N/A') . "</td>";
                              // Column 8: Costo (or N/A)
                              echo "<td>" . ($hasConcentrado ? htmlspecialchars($row['eh_concentrado_costo'] ?? '') : 'N/A') . "</td>";
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['ec_concentrado_nombre']) . '">' . htmlspecialchars($alimento_row['ec_concentrado_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching alimentos: " . $e->getMessage());
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="etapaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $etapas = $stmt_etapas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($etapas as $etapa_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action etapa-option" data-value="' . htmlspecialchars($etapa_row['ec_etapas_nombre']) . '">' . htmlspecialchars($etapa_row['ec_etapas_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching etapas: " . $e->getMessage());

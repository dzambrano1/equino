...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'insert' && isset($_POST['tagid'], $_POST['precio'], $_POST['peso'], $_POST['fecha'])) {
        try {
            // Update equino table with sale information
            ...
            if ($stmt->rowCount() > 0) {
                $response = array(
                    ...
                    'redirect' => 'equino_register_venta.php'
                );
            } else {
                $response = array(
                    'success' => false,
        ...
        try {
            // Clear sale information from equino table
            $stmt = $conn->prepare("UPDATE equino SET 
                precio_venta = NULL, 
                peso_venta = NULL, 
                fecha_venta = NULL,
        ...
        try {
            // Update sale information in equino table
            $stmt = $conn->prepare("UPDATE equino SET 
                precio_venta = ?, 
                peso_venta = ?, 
                fecha_venta = ?

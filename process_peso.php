...
if (isset($_POST['action'])) {
    ...
    try {
        ...
        if ($action === 'insert') {
            ...
            // Execute query
            $stmt = $conn->prepare("INSERT INTO eh_peso (eh_peso_tagid, eh_peso_animal, eh_peso_precio, eh_peso_fecha) VALUES (?, ?, ?, ?)");
        ...
        // UPDATE operation
            ...
            // Execute query
            $stmt = $conn->prepare("UPDATE eh_peso SET eh_peso_tagid = ?, eh_peso_animal = ?, eh_peso_precio = ?, eh_peso_fecha = ? WHERE id = ?");
        ...
        // DELETE operation
            ...
            // Execute query
            $stmt = $conn->prepare("DELETE FROM eh_peso WHERE id = ?");
            $stmt->execute([$id]);

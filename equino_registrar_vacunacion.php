...
<div class="container" id="nav-buttons">
    <div class="container nav-icons-container" id="nav-buttons">
        ...
        <button onclick="window.location.href='./inventario_equino.php'" class="icon-button" data-tooltip="Inventario equino">
            <img src="./images/vacas.png" alt="Inventario" class="nav-icon">
        </button>
        
        <button onclick="window.location.href='./equino_indices.php'" class="icon-button" data-tooltip="Indices equinos">
            <img src="./images/fondo-indexado.png" alt="Inicio" class="nav-icon">
        </button>
        
        <button onclick="window.location.href='./equino_configuracion.php'" class="icon-button" data-tooltip="Configurar Tablas">
...
<div class="container scroll-icons-container">    
    <button class="btn btn-outline-secondary mb-3" onclick="window.location.href='./equino_registrar_pesaje_animal.php'" data-tooltip="Pesaje Carne">
        <img src="./images/carne.png" alt="Pesaje Carne" class="nav-icon">
    </button>
    
    <button class="btn btn-outline-secondary mb-3" onclick="window.location.href='./equino_registrar_pesaje_leche.php'" data-tooltip="Pesaje Leche">
        <img src="./images/leche-cantaro.png" alt="Pesaje Leche" class="nav-icon">
    </button>
    
    <button class="btn btn-outline-secondary mb-3" onclick="window.location.href='./equino_registrar_alimentacion.php'" data-tooltip="Alimentacion">
    ...
    <button  class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./equino_registrar_reproduccion.php'" data-tooltip="Reproduccion">
        <img src="./images/el-embarazo.png" alt="Reproduccion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" onclick="window.location.href='./equino_registrar_poblacion.php'" data-tooltip="Poblacion">
...
<a href="./inventario_equino.php" class="back-btn">
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_aftosa WHERE eh_aftosa_tagid = ? ORDER BY eh_aftosa_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_aftosa ORDER BY eh_aftosa_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($aftosaData as $row) {
        $date = new DateTime($row['eh_aftosa_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_aftosa_dosis']);
        $aftosaFechaLabels[] = $row['eh_aftosa_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_aftosa_costo']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_aftosa_dosis']) * floatval($row['eh_aftosa_costo']);
...
<?php
    ...
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $tagid = $_GET['search'];
        $stmt = $conn->prepare("SELECT nombre FROM equino WHERE tagid = ?");
        $stmt->execute([$tagid]);
        if ($row = $stmt->fetch()) {
    ...
    $aftosaQuery = "SELECT p.*, 
                         ...
                         CAST((p.eh_aftosa_dosis * p.eh_aftosa_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_aftosa p 
                  LEFT JOIN equino v ON p.eh_aftosa_tagid = v.tagid 
                  ORDER BY p.eh_aftosa_fecha DESC";
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'equino_registrar_vacunacion.php';
                        });
                    } else {
...
foreach ($aftosaResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_aftosa_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_aftosa_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_aftosa_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_aftosa_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_aftosa_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-aftosa'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['eh_aftosa_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['eh_aftosa_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['eh_aftosa_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_aftosa_fecha']) . "'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_ibr WHERE eh_ibr_tagid = ? ORDER BY eh_ibr_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_ibr ORDER BY eh_ibr_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($ibrData as $row) {
        $date = new DateTime($row['eh_ibr_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_ibr_dosis']);
        $ibrFechaLabels[] = $row['eh_ibr_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_ibr_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_ibr_dosis']) * floatval($row['eh_ibr_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $ibrQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.eh_ibr_dosis * p.eh_ibr_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_ibr p 
                  LEFT JOIN equino v ON p.eh_ibr_tagid = v.tagid 
                  ORDER BY p.eh_ibr_fecha DESC";
...
foreach ($ibrResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_ibr_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_ibr_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_ibr_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_ibr_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_ibr_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-ibr'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['eh_ibr_producto']) . "'                    
                    data-dosis='" . htmlspecialchars($row['eh_ibr_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['eh_ibr_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_ibr_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_cbr WHERE eh_cbr_tagid = ? ORDER BY eh_cbr_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_cbr ORDER BY eh_cbr_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($cbrData as $row) {
        $date = new DateTime($row['eh_cbr_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_cbr_dosis']);
        $cbrFechaLabels[] = $row['eh_cbr_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_cbr_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_cbr_dosis']) * floatval($row['eh_cbr_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $cbrQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.eh_cbr_dosis * p.eh_cbr_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_cbr p 
                  LEFT JOIN equino v ON p.eh_cbr_tagid = v.tagid 
                  ORDER BY p.eh_cbr_fecha DESC";
...
foreach ($cbrResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_cbr_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_cbr_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_cbr_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_cbr_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_cbr_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-cbr'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['eh_cbr_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['eh_cbr_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['eh_cbr_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_cbr_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_brucelosis WHERE eh_brucelosis_tagid = ? ORDER BY eh_brucelosis_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_brucelosis ORDER BY eh_brucelosis_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($brucelosisData as $row) {
        $date = new DateTime($row['eh_brucelosis_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_brucelosis_dosis']);
        $brucelosisFechaLabels[] = $row['eh_brucelosis_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_brucelosis_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_brucelosis_dosis']) * floatval($row['eh_brucelosis_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $brucelosisQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.eh_brucelosis_dosis * p.eh_brucelosis_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_brucelosis p 
                  LEFT JOIN equino v ON p.eh_brucelosis_tagid = v.tagid 
                  ORDER BY p.eh_brucelosis_fecha DESC";
...
foreach ($brucelosisResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_brucelosis_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_brucelosis_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_brucelosis_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_brucelosis_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_brucelosis_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-brucelosis'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['eh_brucelosis_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['eh_brucelosis_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['eh_brucelosis_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_brucelosis_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_carbunco WHERE eh_carbunco_tagid = ? ORDER BY eh_carbunco_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_carbunco ORDER BY eh_carbunco_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($carbuncoData as $row) {
        $date = new DateTime($row['eh_carbunco_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_carbunco_dosis']);
        $carbuncoFechaLabels[] = $row['eh_carbunco_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_carbunco_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_carbunco_dosis']) * floatval($row['eh_carbunco_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $carbuncoQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.eh_carbunco_dosis * p.eh_carbunco_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_carbunco p 
                  LEFT JOIN equino v ON p.eh_carbunco_tagid = v.tagid 
                  ORDER BY p.eh_carbunco_fecha DESC";
...
foreach ($carbuncoResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_carbunco_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_carbunco_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_carbunco_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_carbunco_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_carbunco_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-carbunco'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['eh_carbunco_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['eh_carbunco_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['eh_carbunco_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_carbunco_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_garrapatas WHERE eh_garrapatas_tagid = ? ORDER BY eh_garrapatas_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_garrapatas ORDER BY eh_garrapatas_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($garrapatasData as $row) {
        $date = new DateTime($row['eh_garrapatas_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_garrapatas_dosis']);
        $garrapatasFechaLabels[] = $row['eh_garrapatas_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_garrapatas_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_garrapatas_dosis']) * floatval($row['eh_garrapatas_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $garrapatasQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.eh_garrapatas_dosis * p.eh_garrapatas_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_garrapatas p 
                  LEFT JOIN equino v ON p.eh_garrapatas_tagid = v.tagid 
                  ORDER BY p.eh_garrapatas_fecha DESC";
...
foreach ($garrapatasResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_garrapatas_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_garrapatas_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_garrapatas_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_garrapatas_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_garrapatas_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-garrapatas'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['eh_garrapatas_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['eh_garrapatas_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['eh_garrapatas_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_garrapatas_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_lombrices WHERE eh_lombrices_tagid = ? ORDER BY eh_lombrices_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_lombrices ORDER BY eh_lombrices_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($lombricesData as $row) {
        $date = new DateTime($row['eh_lombrices_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_lombrices_dosis']);
        $lombricesFechaLabels[] = $row['eh_lombrices_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_lombrices_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_lombrices_dosis']) * floatval($row['eh_lombrices_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $lombricesQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.eh_lombrices_dosis * p.eh_lombrices_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_lombrices p 
                  LEFT JOIN equino v ON p.eh_lombrices_tagid = v.tagid 
                  ORDER BY p.eh_lombrices_fecha DESC";
...
foreach ($lombricesResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_lombrices_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_lombrices_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_lombrices_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_lombrices_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_lombrices_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-lombrices'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['eh_lombrices_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['eh_lombrices_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['eh_lombrices_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_lombrices_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_mastitis WHERE eh_mastitis_tagid = ? ORDER BY eh_mastitis_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_mastitis ORDER BY eh_mastitis_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($mastitisData as $row) {
        $date = new DateTime($row['eh_mastitis_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_mastitis_dosis']);
        $mastitisFechaLabels[] = $row['eh_mastitis_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_mastitis_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_mastitis_dosis']) * floatval($row['eh_mastitis_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $mastitisQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.eh_mastitis_dosis * p.eh_mastitis_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_mastitis p 
                  LEFT JOIN equino v ON p.eh_mastitis_tagid = v.tagid 
                  ORDER BY p.eh_mastitis_fecha DESC";
...
foreach ($mastitisResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_mastitis_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_mastitis_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_mastitis_dosis']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_mastitis_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_mastitis_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-mastitis'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['eh_mastitis_producto']) . "'
                    data-dosis='" . htmlspecialchars($row['eh_mastitis_dosis']) . "'
                    data-costo='" . htmlspecialchars($row['eh_mastitis_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_mastitis_fecha']) . "'
                    data-bs-toggle='modal'

...
<title>Equino - Registro de Vacunación IBR</title>
<!-- Link to the Favicon -->
              ...
              <tbody>
                  ...
                  try {
                      $ibrQuery = "SELECT 
                                          ...
                                          CASE WHEN va.eh_ibr_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_ibr_history
                                      FROM 
                                          equino v
                                      LEFT JOIN 
                                          -- Join with all ibr records per tagid
                                          eh_ibr va ON v.tagid = va.eh_ibr_tagid 
                                      ORDER BY 
                                          va.eh_ibr_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($ibrsData)) {
                          ...
                          try {
                              $configQuery = "SELECT e_vencimiento_ibr FROM e_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['e_vencimiento_ibr'])) {
                                  $vigencia = intval($row_config['e_vencimiento_ibr']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($ibrsData as $row) {
                              ...
                              if ($row['in_ibr_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-ibr" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['eh_ibr_producto'] ?? '') . '" 
                              ...
                              echo "<td>" . htmlspecialchars($row['eh_ibr_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_ibr_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['eh_ibr_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_ibr_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_ibr_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['eh_ibr_fecha'])) {
                                          $ibrDate = new DateTime($row['eh_ibr_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_ibr_fecha']);

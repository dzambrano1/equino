...
try {
    ...
    $sql = "SELECT 
                ...
                SUM(eh_peso_animal * eh_peso_precio) as total_revenue,
                SUM(eh_peso_animal) as total_quantity,
                AVG(eh_peso_precio) as avg_price,
                COUNT(*) as record_count
            FROM eh_peso 
            WHERE eh_peso_fecha IS NOT NULL 
            AND eh_peso_animal > 0 
            AND eh_peso_precio > 0
            GROUP BY DATE_FORMAT(eh_peso_fecha, '%Y-%m')
            ORDER BY DATE_FORMAT(eh_peso_fecha, '%Y-%m') ASC";

...
<title>Equino Register Ventas</title>
<!-- Link to the Favicon -->
           ...
           <table id="soldAnimalsTable" class="table table-striped table-bordered">
               ...
               <tbody>
                   ...
                   try {
                     ...
                     $soldQuery = "SELECT v.*, vp.eh_peso_animal, vp.eh_peso_precio, vp.eh_peso_fecha as peso_fecha
                               FROM equino v
                               LEFT JOIN (
                                   SELECT vp1.*
                                   FROM eh_peso vp1
                                   INNER JOIN (
                                       SELECT eh_peso_tagid, MAX(eh_peso_fecha) as max_fecha
                                       FROM eh_peso
                                       GROUP BY eh_peso_tagid
                                   ) vp2 ON vp1.eh_peso_tagid = vp2.eh_peso_tagid AND vp1.eh_peso_fecha = vp2.max_fecha
                               ) vp ON v.tagid = vp.eh_peso_tagid
                               WHERE v.estatus = 'Vendido'
                               ORDER BY vp.eh_peso_fecha DESC";
                     ...
                     if (empty($soldData)) {
                         ...
                         foreach ($soldData as $row) {
                             ...
                             echo "<td class='text-center'>" . htmlspecialchars($row['eh_peso_precio'] ?? 'N/A') . "</td>";
                             echo "<td class='text-center'>" . htmlspecialchars($row['eh_peso_animal'] ?? 'N/A') . "</td>";                            
                             echo "</tr>";
                         }
                     }
           ...
           <table id="availableAnimalsTable" class="table table-striped table-bordered">
               ...
               <tbody>
                   ...
                   try {
                     ...
                     $availableQuery = "SELECT v.*, vp.eh_peso_animal, vp.eh_peso_precio, vp.eh_peso_fecha as peso_fecha
                               FROM equino v
                               LEFT JOIN (
                                   SELECT vp1.*
                                   FROM eh_peso vp1
                               ...
                               ORDER BY v.estatus DESC, vp.eh_peso_fecha DESC";                              
    ...
    $('#saveNewVenta').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Send AJAX request to update equino record with sale information
                $.ajax({

...
<title>Equino - Registro de Vacunación Aftosa</title>
<!-- Link to the Favicon -->
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT ec_aftosa_vacuna FROM ec_aftosa ORDER BY ec_aftosa_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['ec_aftosa_vacuna']) . '">' . htmlspecialchars($vacuna_row['ec_aftosa_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $aftosaQuery = "SELECT 
                                          ...
                                          va.eh_aftosa_producto,
                                          va.eh_aftosa_dosis,
                                          va.eh_aftosa_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.eh_aftosa_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_aftosa_history
                                      FROM 
                                          equino v
                                      LEFT JOIN 
                                          -- Join with all aftosa records per tagid
                                          eh_aftosa va ON v.tagid = va.eh_aftosa_tagid 
                                      ORDER BY 
                                          va.eh_aftosa_fecha DESC, v.tagid ASC;";
                          ...
                          foreach ($aftosasData as $row) {
                              ...
                              if ($row['in_aftosa_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-aftosa" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['eh_aftosa_producto'] ?? '') . '" 
                              ...
                              echo "<td>" . htmlspecialchars($row['eh_aftosa_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_aftosa_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['eh_aftosa_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_aftosa_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_aftosa_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['eh_aftosa_fecha'])) {
                                          $aftosaDate = new DateTime($row['eh_aftosa_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_aftosa_fecha']);
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['ec_aftosa_vacuna']) . '">' . htmlspecialchars($vacuna_row['ec_aftosa_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());

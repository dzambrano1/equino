...
try {
    ...
    $sql = "
        ...
        FROM eh_concentrado
        WHERE eh_concentrado_racion > 0 AND eh_concentrado_costo > 0 -- Consider only valid cost entries
        ORDER BY eh_concentrado_tagid, eh_concentrado_fecha ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $fecha = $row['eh_concentrado_fecha'];
            $daily_cost = (float)$row['eh_concentrado_racion'] * (float)$row['eh_concentrado_costo'];

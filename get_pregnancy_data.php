...
try {
    ...
    $sql = "SELECT 
                DATE_FORMAT(eh_gestacion_fecha, '%Y-%m') as month,
                COUNT(*) as pregnancy_count,
                AVG(eh_gestacion_numero) as avg_pregnancy_number,
                MAX(eh_gestacion_numero) as max_pregnancy_number,
                COUNT(*) as record_count
            FROM eh_gestacion 
            WHERE eh_gestacion_fecha IS NOT NULL 
            AND eh_gestacion_numero > 0
            GROUP BY DATE_FORMAT(eh_gestacion_fecha, '%Y-%m')
            ORDER BY DATE_FORMAT(eh_gestacion_fecha, '%Y-%m') ASC";

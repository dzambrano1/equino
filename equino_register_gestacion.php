...
$chartData = [];
try {
    $chartQuery = "SELECT DATE_FORMAT(eh_gestacion_fecha, '%Y-%m') as month_year, COUNT(*) as count 
                     FROM eh_gestacion 
                     GROUP BY month_year 
...
<title>Equino Register Gestacion</title>
<!-- Link to the Favicon -->
                  ...
                  try {
                      // Query to get all Female animals and their LATEST gestacion record (if any)
                        $gestacionQuery = "
                            SELECT
                                ...
                                ins.eh_gestacion_fecha,
                                ins.eh_gestacion_tagid, -- Will be NULL if no gestacion
                                ins.eh_gestacion_numero
                            FROM
                                equino v
                            LEFT JOIN eh_gestacion ins
                              ON v.tagid = ins.eh_gestacion_tagid
                            LEFT JOIN eh_gestacion ins_check
                              ON ins.eh_gestacion_tagid = ins_check.eh_gestacion_tagid
                             AND ins.eh_gestacion_fecha < ins_check.eh_gestacion_fecha -- Check for a newer record
                            ...
                            ORDER BY
                                ...
                                ins.eh_gestacion_fecha DESC, -- Then by latest insemination date
                      ...
                      if (empty($gestacionData)) {
                          ...
                          foreach ($gestacionData as $row) {
                              ...
                              $gestacionFecha = $row['eh_gestacion_fecha'] ?? null;
                              ...
                              echo "<td class='text-center'>" . ($hasgestacion ? htmlspecialchars($row['eh_gestacion_numero'] ?? '') : 'N/A') . "</td>";                              

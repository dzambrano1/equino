...
if (!empty($fetched_orders)) {
    ...
    $items_query = "SELECT od.*, v.image AS equino_image -- Use alias for equino image 
                    FROM order_details od 
                    LEFT JOIN equino v ON od.tagid = v.tagid 
    ...
    foreach ($fetched_orders as $order) {
        ...
        try {
            ...
            foreach ($items_result as $item) {
                 // Prefer order_details image if exists, otherwise use equino image
                $item['image'] = $item['image'] ?: ($item['equino_image'] ?: './images/default_animal.png');
                unset($item['equino_image']); // Clean up alias
                $order_details[] = $item;
            }

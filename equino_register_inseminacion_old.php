...
$chartData = [];
try {
    $chartQuery = "SELECT DATE_FORMAT(eh_inseminacion_fecha, '%Y-%m') as month_year, COUNT(*) as count 
...
<title>Equino Register Inseminacion</title>
<!-- Link to the Favicon -->
                  ...
                  try {
                      // Query to get all Female animals and their LATEST insemination record (if any)
                        $inseminacionQuery = "
                            SELECT
                                ...
                                ins.eh_inseminacion_fecha,
                                ins.eh_inseminacion_tagid, -- Will be NULL if no insemination
                                ins.eh_inseminacion_numero,
                                ins.eh_inseminacion_costo
                            FROM
                                equino v
                            LEFT JOIN eh_inseminacion ins
                              ON v.tagid = ins.eh_inseminacion_tagid
                            LEFT JOIN eh_inseminacion ins_check
                              ON ins.eh_inseminacion_tagid = ins_check.eh_inseminacion_tagid
                             AND ins.eh_inseminacion_fecha < ins_check.eh_inseminacion_fecha -- Check for a newer record
                            ...
                            ORDER BY
                                ...
                                ins.eh_inseminacion_fecha DESC, -- Then by latest insemination date
                      ...
                      if (empty($inseminacionData)) {
                          ...
                          foreach ($inseminacionData as $row) {
                              ...
                              $inseminacionFecha = $row['eh_inseminacion_fecha'] ?? null;
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-success btn-sm" 
                                              ...
                                              data-tagid-prefill="'.htmlspecialchars($row['equino_tagid'] ?? '').'"  -- Pass tagid to modal
                              ...
                              echo "<td class='text-center'>" . htmlspecialchars($row['equino_tagid'] ?? 'N/A') . "</td>"; // Now using equino_tagid
                              echo "<td class='text-center'>" . ($hasInseminacion ? htmlspecialchars($row['eh_inseminacion_numero'] ?? '') : 'N/A') . "</td>";
                              echo "<td class='text-center'>" . ($hasInseminacion ? htmlspecialchars($row['eh_inseminacion_costo'] ?? '') : 'N/A') . "</td>";

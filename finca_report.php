        ...
        if ($row_count > 0) {
            ...
            $monthlyQuery = "
                SELECT 
                    ...
                    AVG(eh_peso_precio) as avg_price,
                    COUNT(DISTINCT eh_peso_tagid) as animal_count
                FROM eh_peso
                GROUP BY month
                ORDER BY month ASC
                LIMIT 12";
                    ...
                    $monthlyMilkQuery = "
                        WITH RankedSamples AS (
                            SELECT 
                                ...
                                eh_leche_peso,
                                eh_leche_precio,
                                LEAD(eh_leche_fecha) OVER (PARTITION BY eh_leche_tagid ORDER BY eh_leche_fecha) as next_fecha
                            FROM eh_leche
                        ),
                        ProductionCalculations AS (
                            SELECT 
                                ...
                                eh_leche_precio,
                                COALESCE(
                                    DATEDIFF(next_fecha, eh_leche_fecha),
                                    DATEDIFF(CURRENT_DATE, eh_leche_fecha)
                                ) as days_until_next,
                                eh_leche_peso * 
                                COALESCE(
                                    DATEDIFF(next_fecha, eh_leche_fecha),
                        ...
                        SELECT 
                            DATE_FORMAT(eh_leche_fecha, '%Y-%m') as month,
                            SUM(period_production) as total_production,
                            AVG(eh_leche_precio) as avg_price,
                            COUNT(DISTINCT eh_leche_tagid) as animal_count

...
$avgPesoQuery = "
    ...
    FROM 
        eh_peso 
    WHERE 
        eh_peso_tagid IN (" . str_repeat('?,', count($tagids) - 1) . "?) 
    GROUP BY 
        peso_month 
    ORDER BY 
        peso_month ASC
...
window.generateAndUploadPDF = async function(tagid) {
    console.log('generateAndUploadPDF called with tagid:', tagid);
    if (!tagid) {
    ...
    Swal.fire({
        title: 'Preparando Veterinario IA...',
        html: 'Por favor espere mientras:<br>1. Se genera el reporte PDF<br>2. Se sube a la IA<br>3. Se prepara el chat',
    ...
    try {
        // Step 1: Generate PDF
        console.log('Step 1: Generating PDF for tagid:', tagid);
        const pdfResponse = await fetch(`equino_report.php?tagid=${encodeURIComponent(tagid)}`, {
            method: 'GET',
            headers: {
        ...
        if (!pdfResponse.ok) {
            ...
            console.error('PDF generation failed. Status:', pdfResponse.status, 'Response:', errorText);
            throw new Error(`Error del servidor (${pdfResponse.status}): ${errorText.substring(0, 200)}`);
        }
        ...
        try {
            ...
            console.log('Raw PDF response:', responseText.substring(0, 500));
        ...
        if (!pdfData.success) {
            throw new Error(pdfData.message || 'Error generando PDF');
        }
        
        // Step 2: Upload PDF to ChatPDF
        console.log('Step 2: Uploading PDF to ChatPDF:', pdfData.filename);
        Swal.update({
            html: 'PDF generado ✓<br>Subiendo a ChatPDF IA...'
        });
        
        // Get the PDF file and upload it
        const pdfFileResponse = await fetch(`reports/${pdfData.filename}`);
        ...
        formData.append('file', pdfBlob, pdfData.filename);
        ...
        // Better error handling for upload response
        let uploadData;
        try {
            ...
            throw new Error('Error al procesar respuesta de ChatPDF API. Verifica tu API key y conexión.');
        }
        
        console.log('ChatPDF upload response:', uploadData);
        
        // ChatPDF API returns { sourceId: "..." } on success, or { error: "..." } on failure
        if (uploadData.error || !uploadData.sourceId) {
            throw new Error(uploadData.error || 'Error subiendo a ChatPDF - No sourceId returned');
        }
        
        // Step 3: Open chat modal
        console.log('Step 3: Opening chat modal with sourceId:', uploadData.sourceId);
        Swal.close();
        
        // Open the chat modal immediately
        openChatPDFWindow(uploadData.sourceId, pdfData.filename);
        ...
        Swal.fire({
            ...
            title: 'Error',
            html: `
                <p><strong>No se pudo preparar el Veterinario IA</strong></p>
                <p>${error.message}</p>
                <p style="font-size: 0.9em; color: #666;">
                    Por favor verifica:<br>
                    • Conexión a internet<br>
                    • Configuración de ChatPDF API<br>
                    • Que el animal existe en el sistema
                </p>
            `,
...
console.log('generateAndUploadPDF function registered:', typeof window.generateAndUploadPDF);

// Test function to open modal directly (for debugging)
window.testChatPDFModal = function() {
    console.log('Testing ChatPDF modal...');
    openChatPDFWindow('test-source-id', 'test_animal_123.pdf');
};
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="etapa" id="updateEtapa" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_etapa = "SELECT DISTINCT etapa FROM equino ORDER BY etapa";

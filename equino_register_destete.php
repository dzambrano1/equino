...
$chartData = [];
try {
    $chartQuery = "SELECT DATE_FORMAT(eh_destete_fecha, '%Y-%m') as month_year, COUNT(*) as count 
                     FROM eh_destete 
                     GROUP BY month_year 
...
<title>Equino Register Destete</title>
<!-- Link to the Favicon -->
                  ...
                  try {
                      // Query to get all Female animals and their LATEST parto record (if any)
                        $desteteQuery = "
                            SELECT
                                ...
                                ins.eh_destete_fecha,
                                ins.eh_destete_tagid, -- Will be NULL if no parto
                                ins.eh_destete_peso
                            FROM
                                equino v
                            LEFT JOIN eh_destete ins
                              ON v.tagid = ins.eh_destete_tagid
                            LEFT JOIN eh_destete ins_check
                              ON ins.eh_destete_tagid = ins_check.eh_destete_tagid
                             AND ins.eh_destete_fecha < ins_check.eh_destete_fecha -- Check for a newer record
                            ...
                            ORDER BY
                                ...
                                ins.eh_destete_fecha DESC, -- Then by latest insemination date
                      ...
                      if (empty($desteteData)) {
                          ...
                          foreach ($desteteData as $row) {
                              ...
                              $desteteFecha = $row['eh_destete_fecha'] ?? null;
                              ...
                              echo "<td class='text-center'>" . ($hasdestete ? htmlspecialchars($row['eh_destete_peso'] ?? '') : 'N/A') . "</td>";                              

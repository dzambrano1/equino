...
try {
    ...
    $sql = "
        SELECT
            ...
            eh_sal_costo
        FROM eh_sal
        WHERE eh_sal_racion > 0 AND eh_sal_costo > 0 -- Consider only valid cost entries
        ORDER BY eh_sal_tagid, eh_sal_fecha_inicio ASC;
    ";
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $tagid = $row['eh_sal_tagid'];
            $fecha_inicio = $row['eh_sal_fecha_inicio'];
            $fecha_fin = $row['eh_sal_fecha_fin'];
            
            // Calculate daily cost: racion (daily) × costo
            // Since racion is already a daily figure, this is the cost per day
            $daily_cost = (float)$row['eh_sal_racion'] * (float)$row['eh_sal_costo'];

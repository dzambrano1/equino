...
<title>Equino Register Decesos</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
   ...
   <!-- Setup Database Fields Button -->
   <div class="container text-center mb-3">
       <button id="setupFieldsBtn" class="btn btn-info btn-sm">
           <i class="fas fa-database me-2"></i>Verificar Campos de Base de Datos
       </button>
   </div>
          ...
          <table id="deceasedAnimalsTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      ...
                      <th class="text-center">F. Deceso</th>
               ...
               <tbody>
                   <?php
                   try {
                       // Query to get deceased animals - using only existing fields for now
                       $deceasedQuery = "SELECT v.*
                                     FROM equino v
                       ...
                       if (empty($deceasedData)) {
                           ...
                           foreach ($deceasedData as $row) {
                               ...
                               echo "<td class='text-center'>" . htmlspecialchars($row['deceso_fecha'] ?? 'N/A') . "</td>";
...
$(document).ready(function() {
    ...
    if ($('#deceasedAnimalsTable tbody tr').length > 1) { // More than just the "no records" row
        ...
        lengthMenu: [
         ...
         order: [[4, 'desc']], // F. Deceso column (5th column, 0-indexed)
        ...
        buttons: [
         ...
         columnDefs: [
             {
                 targets: [4], // F. Deceso column (5th column, 0-indexed)
 ...
 <script>
 $(document).ready(function() {
     // Handle setup fields button click
     $('#setupFieldsBtn').click(function() {
         // Show loading state
         Swal.fire({
             title: 'Verificando base de datos...',
             text: 'Por favor espere mientras se verifican los campos necesarios',
         ...
         // Send AJAX request to setup database fields
         $.ajax({
             url: 'setup_deceso_fields.php',
             type: 'GET',
             dataType: 'json',
             success: function(response) {
                 if (response.success) {
                     Swal.fire({
                         title: '¡Campos verificados!',
                     ...
                     Swal.fire({
                         title: 'Error',
                         text: response.message || 'Ha ocurrido un error al verificar los campos',
                         icon: 'error',
     ...
     $('#saveNewDeceso').click(function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                // Send AJAX request to update the equino record
                $.ajax({

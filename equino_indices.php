...
<title>Dashboard - Indices Equinos</title>
<!-- Link to the Favicon -->
...
<div class="dashboard-container">
    ...
    <div class="dashboard-header">
        <h1 class="dashboard-title">
            ...
            Tablero de Control Indices Equinos
        </h1>
    </div>
...
async function loadChart(chartType, canvasId, isModal = false) {
    ...
    try {
        ...
        // Special handling for weaning chart - check if data has required fields
        if (chartType === 'weaning' && data.length > 0) {
            const firstItem = data[0];
            if (!firstItem.hasOwnProperty('month') || !firstItem.hasOwnProperty('weaning_count')) {
                console.warn('Weaning data missing required fields:', firstItem);
                if (loadingDiv) {
                    loadingDiv.innerHTML = '<div class="alert alert-warning"><i class="fas fa-exclamation-triangle me-2"></i>Formato de datos incorrecto para destetes</div>';
                }
                return;
            }
        }
        ...
        // Debug logging for weaning chart
        if (chartType === 'weaning') {
            console.log('Weaning chart data processed:', chartData);
        }
...
function processChartData(data, chartType, config) {
    ...
    switch(chartType) {
        ...
        case 'weaning':
            ...
            values = data.map(item => parseInt(item.weaning_count || item.count || 0));
            
            // Store raw data for tooltips with weaning details
            rawDataArray = data.map(item => ({
                month: item.month,
                weaning_count: parseInt(item.weaning_count || item.count || 0),
...
function getChartType(chartType) {
    ...
    if (chartType === 'concentradoExpense' || chartType === 'melazaExpense' || chartType === 'salExpense') {
...
function getChartOptions(chartType, isModal = false, hasCumulativeData = false) {
    const baseOptions = {
        ...
        scales: {
            y: {
                ...
                beginAtZero: true,
                title: {
                    display: chartType === 'concentradoExpense' || chartType === 'melazaExpense' || chartType === 'salExpense',
                    text: chartType === 'concentradoExpense' ? 'Gastos ($)' : chartType === 'melazaExpense' ? 'Gastos ($)' : chartType === 'salExpense' ? 'Gastos ($)' : 'Gastos Mensuales ($)',
                    color: '#666',
                    font: {
                        size: isModal ? 14 : 12,
                        weight: 'bold'
    ...
    if (hasCumulativeData && (chartType === 'concentradoExpense' || chartType === 'melazaExpense' || chartType === 'salExpense')) {
...
if (chartType === 'concentradoExpense') {
    ...
    const tooltipChartTypes = ['deaths', 'discards', 'weaning', 'insemination', 'concentradoExpense', 'melazaExpense', 'salExpense', 'financialSummary', 'allExpenses'];
    if (tooltipChartTypes.includes(chartType)) {
        baseOptions.plugins.tooltip = {
            ...
            callbacks: {
                ...
                label: function(context) {
                    try {
                        ...
                        // Special handling for weaning chart
                        const isWeaningChart = chartType === 'weaning' || 
                                              context.chart.canvas.id.includes('weaning') ||
                                              context.chart.canvas.id === 'modalChart' && currentExpandedChart === 'weaning';
                        
                        if (isWeaningChart) {
                            if (dataset && dataset.rawData && dataset.rawData[dataIndex]) {
                                const weaningData = dataset.rawData[dataIndex];
                                const lines = [
                                    `Destetes: ${weaningData.weaning_count}`,
                                    `Peso Promedio: ${weaningData.avg_weight.toFixed(1)} kg`,
                                    `Peso Total: ${weaningData.total_weight.toFixed(1)} kg`,
                                    `Registros: ${weaningData.record_count}`
                                ];
                                
                                if (weaningData.tagids && weaningData.tagids.trim() !== '') {
                                    lines.push(`Tag IDs: ${weaningData.tagids}`);
                                }
                                
                                return lines;
                            }
                            return [
                                `Destetes: ${context.parsed.y}`
                            ];
                        }
            ...
            filter: function(tooltipItem) {
                ...
                if (isConcentradoChart || isMelazaChart || isSalChart) {

...
<title>Equino Configuracion Melaza</title>
<!-- Link to the Favicon -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
  ...
  <!-- DataTable for ec_melaza records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="melazaTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $melazaQuery = "SELECT * FROM ec_melaza";
                      ...
                      if (empty($melazasData)) {
                          ...
                          foreach ($melazasData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-melaza" 
                                              ...
                                              data-melaza="' . htmlspecialchars($row['ec_melaza_nombre'] ?? '') . '" 
                                              data-etapa="' . htmlspecialchars($row['ec_melaza_etapa'] ?? '') . '" 
                                              data-racion="' . htmlspecialchars($row['ec_melaza_racion'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['ec_melaza_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['ec_melaza_vigencia'] ?? '') . '"
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['ec_melaza_nombre'] ?? '') . "</td>";
                              // Columna 2: Etapa
                              echo "<td>" . htmlspecialchars($row['ec_melaza_etapa'] ?? '') . "</td>";
                              
                              // Column 3: Dosis
                              echo "<td>" . htmlspecialchars($row['ec_melaza_racion'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Costo
                              echo "<td>" . htmlspecialchars($row['ec_melaza_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 5: Vigencia
                              echo "<td>" . htmlspecialchars($row['ec_melaza_vigencia'] ?? 'N/A') . "</td>";
                        ...
                        <form id="editMelazaForm">
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select" id="edit_melaza" name="melaza" required>
                                    ...
                                    $sql_names = "SELECT DISTINCT ec_melaza_nombre FROM ec_melaza ORDER BY ec_melaza_nombre ASC";
                                    $stmt_names = $conn->prepare($sql_names);
                                    $stmt_names->execute();
                                    $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($names as $name_row) {
                                        echo '<option value="' . htmlspecialchars($name_row['ec_melaza_nombre']) . '">' . htmlspecialchars($name_row['ec_melaza_nombre']) . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>

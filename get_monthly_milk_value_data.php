...
try {
    ...
    // Each eh_leche_peso is treated as daily average production for that month
    ...
    $query = "
        SELECT 
            ...
            ROUND(SUM(
                eh_leche_peso * 
                DAY(LAST_DAY(eh_leche_fecha)) * 
                eh_leche_precio
            ), 2) AS total_milk_value,
            UNIX_TIMESTAMP(MIN(eh_leche_fecha)) as month_timestamp
        FROM eh_leche 
        WHERE eh_leche_peso > 0 AND eh_leche_precio > 0 
        GROUP BY DATE_FORMAT(eh_leche_fecha, '%Y-%m')
        ORDER BY month ASC
    ";

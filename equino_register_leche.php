...
try {
    ...
    $lecheQuery = "SELECT 
                        eh_leche_fecha_inicio,
                        eh_leche_fecha_fin,
                        eh_leche_peso,
                        eh_leche_precio,
                        eh_leche_tagid
                    FROM eh_leche 
                    WHERE eh_leche_fecha_inicio IS NOT NULL 
                    AND eh_leche_fecha_fin IS NOT NULL 
                    AND eh_leche_peso IS NOT NULL 
                    AND eh_leche_precio IS NOT NULL
                    ORDER BY eh_leche_fecha_inicio ASC";
    ...
    foreach ($lecheData as $leche) {
        $startDate = new DateTime($leche['eh_leche_fecha_inicio']);
        $endDate = new DateTime($leche['eh_leche_fecha_fin']);
        
        // Calculate total production value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $leche['eh_leche_peso'] * $leche['eh_leche_precio'] * $totalDays;
...
<title>equino Registro Leche</title>
<!-- Link to the Favicon -->
                  ...
                  try {
                      // Query to get all Female Animals and ALL their milk records (if any)
                        $milkQuery = "
                            SELECT
                                ...
                                l.eh_leche_fecha_inicio,
                                l.eh_leche_tagid,         -- Matches equino_tagid if milk record exists
                                l.eh_leche_peso,
                                l.eh_leche_precio,
                                -- Calculate total_value only if l.id is not null
                                CASE WHEN l.id IS NOT NULL THEN CAST((l.eh_leche_peso * l.eh_leche_precio) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                equino b
                            LEFT JOIN
                                eh_leche l ON b.tagid = l.eh_leche_tagid -- Join ALL matching milk records
                            WHERE
                      ...
                      if (empty($milksData)) {
                          ...
                          foreach ($milksData as $row) {
                              ...
                              $lecheFecha = $row['eh_leche_fecha_inicio'] ?? null;
                              ...
                              if ($hasLeche) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-peso" 
                                                  ...
                                                  data-peso="'.htmlspecialchars($row['eh_leche_peso'] ?? '').'" 
                                  ...
                                  echo '        <button class="btn btn-danger btn-sm delete-peso" 
                                                  data-id="'.htmlspecialchars($row['leche_id'] ?? '').'" 
                                                  data-tagid="'.htmlspecialchars($row['eh_leche_tagid'] ?? '').'" -- Pass tagid for context
                              ...
                              echo "<td>" . htmlspecialchars($row['equino_tagid'] ?? 'N/A') . "</td>"; // Use equino_tagid for consistency
                              // Column 5: Leche (kg)
                              echo "<td>" . ($hasLeche ? htmlspecialchars($row['eh_leche_peso'] ?? '') : 'N/A') . "</td>";
                              // Column 6: Precio ($/kg)
                              echo "<td>" . ($hasLeche ? htmlspecialchars($row['eh_leche_precio'] ?? '') : 'N/A') . "</td>";

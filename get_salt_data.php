...
try {
    ...
    switch ($type) {
        case 'monthly_expense':
            ...
            $query = "
                SELECT 
                    DATE_FORMAT(eh_sal_fecha, '%Y-%m') as month,
                    ROUND(SUM(eh_sal_racion * eh_sal_costo), 2) as total_expense
                FROM eh_sal 
                WHERE eh_sal_fecha IS NOT NULL 
                GROUP BY DATE_FORMAT(eh_sal_fecha, '%Y-%m')
                ORDER BY month ASC
            ";
            break;
        ...
        case 'monthly_weight':
            ...
            $query = "
                SELECT 
                    DATE_FORMAT(eh_sal_fecha, '%Y-%m') as month,
                    ROUND(SUM(eh_sal_racion), 2) as total_weight
                FROM eh_sal 
                WHERE eh_sal_fecha IS NOT NULL 
                GROUP BY DATE_FORMAT(eh_sal_fecha, '%Y-%m')
                ORDER BY month ASC
        ...
        case 'monthly_feed_weight':
            ...
            $query = "
                SELECT 
                    DATE_FORMAT(eh_sal_fecha, '%Y-%m') as month,
                    ROUND(SUM(eh_sal_racion), 2) as total_feed_kg
                FROM eh_sal 
                WHERE eh_sal_fecha IS NOT NULL 
                GROUP BY DATE_FORMAT(eh_sal_fecha, '%Y-%m')
                ORDER BY month ASC
        ...
        case 'animal_weight':
            ...
            $query = "
                SELECT 
                    DATE_FORMAT(eh_sal_fecha, '%Y-%m') as month,
                    COUNT(DISTINCT eh_sal_tagid) * 400 as total_weight
                FROM eh_sal 
                WHERE eh_sal_fecha IS NOT NULL 
                GROUP BY DATE_FORMAT(eh_sal_fecha, '%Y-%m')
                ORDER BY month ASC
        ...
        default:
            ...
            $query = "
                SELECT 
                    DATE_FORMAT(s.eh_sal_fecha, '%Y-%m') as month,
                    v.tagid,
                    v.nombre as animal_nombre,
                    ROUND(AVG(eh_sal_costo * eh_sal_racion), 2) as average_cost
                FROM 
                    eh_sal s
                    LEFT JOIN equino v ON s.eh_sal_tagid = v.tagid 
                GROUP BY 
                    DATE_FORMAT(s.eh_sal_fecha, '%Y-%m'),
                    v.tagid,
                    v.nombre
                ORDER BY 
                    month ASC, 
    ...
    if ($type === '' || $type === null) {
        ...
        $formattedData = array_map(function($row) {
            return [
                ...
                'eh_sal_cantidad' => $row['average_cost']
            ];
        }, $data);
    } else {

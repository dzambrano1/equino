...
<div class="container scroll-icons-container">
    ...
    <button class="btn btn-outline-secondary mb-3" onclick="window.location.href='./equino_registrar_vacunacion.php'" data-tooltip="Salud">
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_concentrado WHERE eh_concentrado_tagid = ? ORDER BY eh_concentrado_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_concentrado ORDER BY eh_concentrado_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($concentradoData as $row) {
        $date = new DateTime($row['eh_concentrado_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_concentrado_racion']);
        $concentradoFechaLabels[] = $row['eh_concentrado_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_concentrado_costo']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_concentrado_racion']) * floatval($row['eh_concentrado_costo']);
...
<?php
    ...
    $concentradoQuery = "SELECT p.*, 
                         ...
                         CAST((p.eh_concentrado_racion * p.eh_concentrado_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_concentrado p 
                  LEFT JOIN equino v ON p.eh_concentrado_tagid = v.tagid 
                  ORDER BY p.eh_concentrado_fecha DESC";
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'equino_registrar_alimentacion.php';
                        });
                    } else {
...
foreach ($concentradoResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_concentrado_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_concentrado_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_concentrado_etapa']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_concentrado_racion']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_concentrado_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_concentrado_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-concentrado'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-tagid='" . htmlspecialchars($row['eh_concentrado_tagid']) . "'
                    data-producto='" . htmlspecialchars($row['eh_concentrado_producto']) . "'
                    data-etapa='" . htmlspecialchars($row['eh_concentrado_etapa']) . "'
                    data-racion='" . htmlspecialchars($row['eh_concentrado_racion']) . "'
                    data-costo='" . htmlspecialchars($row['eh_concentrado_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_concentrado_fecha']) . "'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_melaza WHERE eh_melaza_tagid = ? ORDER BY eh_melaza_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_melaza ORDER BY eh_melaza_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($melazaData as $row) {
        $date = new DateTime($row['eh_melaza_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_melaza_racion']);
        $melazaFechaLabels[] = $row['eh_melaza_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_melaza_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_melaza_racion']) * floatval($row['eh_melaza_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $melazaQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.eh_melaza_racion * p.eh_melaza_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_melaza p 
                  LEFT JOIN equino v ON p.eh_melaza_tagid = v.tagid 
                  ORDER BY p.eh_melaza_fecha DESC";
...
foreach ($melazaResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_melaza_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_melaza_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_melaza_etapa']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_melaza_racion']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_melaza_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_melaza_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-melaza'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['eh_melaza_producto']) . "'
                    data-etapa='" . htmlspecialchars($row['eh_melaza_etapa']) . "'
                    data-racion='" . htmlspecialchars($row['eh_melaza_racion']) . "'
                    data-costo='" . htmlspecialchars($row['eh_melaza_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_melaza_fecha']) . "'
                    data-bs-toggle='modal'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_sal WHERE eh_sal_tagid = ? ORDER BY eh_sal_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_sal ORDER BY eh_sal_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($salData as $row) {
        $date = new DateTime($row['eh_sal_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_sal_racion']);
        $salFechaLabels[] = $row['eh_sal_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_sal_costo']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_sal_racion']) * floatval($row['eh_sal_costo']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $salQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.eh_sal_racion * p.eh_sal_costo) AS DECIMAL(10,2)) as total_value
                  FROM eh_sal p 
                  LEFT JOIN equino v ON p.eh_sal_tagid = v.tagid 
                  ORDER BY p.eh_sal_fecha DESC";
...
foreach ($salResult as $row) {
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_sal_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_sal_producto']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_sal_etapa']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_sal_racion']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_sal_costo']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_sal_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-sal'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-producto='" . htmlspecialchars($row['eh_sal_producto']) . "'
                    data-etapa='" . htmlspecialchars($row['eh_sal_etapa']) . "'
                    data-racion='" . htmlspecialchars($row['eh_sal_racion']) . "'
                    data-costo='" . htmlspecialchars($row['eh_sal_costo']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_sal_fecha']) . "'
                    data-bs-toggle='modal'

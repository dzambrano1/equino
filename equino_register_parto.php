...
$chartData = [];
try {
    $chartQuery = "SELECT DATE_FORMAT(eh_parto_fecha, '%Y-%m') as month_year, COUNT(*) as count 
                     FROM eh_parto 
                     GROUP BY month_year 
...
<title>equino Register Parto</title>
<!-- Link to the Favicon -->
                  ...
                  try {
                      // Query to get all Female animals and their LATEST parto record (if any)
                        $partoQuery = "
                            SELECT
                                ...
                                ins.id AS parto_id,
                                ins.eh_parto_fecha,
                                ins.eh_parto_tagid, -- Will be NULL if no parto
                                ins.eh_parto_numero
                            FROM
                                equino v
                            LEFT JOIN eh_parto ins
                              ON v.tagid = ins.eh_parto_tagid
                            LEFT JOIN eh_parto ins_check
                              ON ins.eh_parto_tagid = ins_check.eh_parto_tagid
                             AND ins.eh_parto_fecha < ins_check.eh_parto_fecha -- Check for a newer record
                            ...
                            ORDER BY
                                ...
                                ins.eh_parto_fecha DESC, -- Then by latest insemination date
                      ...
                      if (empty($partoData)) {
                          ...
                          foreach ($partoData as $row) {
                              ...
                              $partoFecha = $row['eh_parto_fecha'] ?? null;
                              ...
                              echo "<td class='text-center'>" . ($hasparto ? htmlspecialchars($row['eh_parto_numero'] ?? '') : 'N/A') . "</td>";                              

...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_peso WHERE eh_peso_tagid = ? ORDER BY eh_peso_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_peso ORDER BY eh_peso_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($pesoData as $row) {
        $date = new DateTime($row['eh_peso_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_peso_animal']);
        $pesoFechaLabels[] = $row['eh_peso_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_peso_precio']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_peso_animal']) * floatval($row['eh_peso_precio']);
...
<?php
    ...
    $pesoQuery = "SELECT p.*, 
                         ...
                         CAST((p.eh_peso_animal * p.eh_peso_precio) AS DECIMAL(10,2)) as total_value
                  FROM eh_peso p 
                  LEFT JOIN equino v ON p.eh_peso_tagid = v.tagid 
                  ORDER BY p.eh_peso_fecha DESC";
...
foreach ($pesosResult as $row) {
    if ($row['eh_peso_tagid'] == '3000') {
        error_log("Debug - TagID 3000:");
        error_log("peso_animal: " . $row['eh_peso_animal']);
        error_log("peso_precio: " . $row['eh_peso_precio']);
        error_log("total_value: " . $row['total_value']);
    }
    echo "<tr>";    
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_peso_tagid']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_peso_animal']) . "</td>";
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_peso_precio']) . "</td>";
    echo "<td class='text-center'>" . number_format($row['total_value'], 2, '.', ',') . "</td>"; // Format with 2 decimal places
    echo "<td class='text-center'>" . htmlspecialchars($row['eh_peso_fecha']) . "</td>";
    echo "<td class='text-center'>

            <button class='btn btn-success btn-sm edit-peso'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-peso='" . htmlspecialchars($row['eh_peso_animal']) . "'
                    data-precio='" . htmlspecialchars($row['eh_peso_precio']) . "'
                    data-fecha='" . htmlspecialchars($row['eh_peso_fecha']) . "'

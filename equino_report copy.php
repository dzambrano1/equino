...
$sql_weight = "SELECT eh_peso_fecha, eh_peso_animal, eh_peso_precio FROM eh_peso WHERE eh_peso_tagid = ? ORDER BY eh_peso_fecha DESC";
...
if ($result_weight->num_rows > 0) {
    ...
    while ($row = $result_weight->fetch_assoc()) {
        $data[] = array($row['eh_peso_fecha'], $row['eh_peso_animal'], $row['eh_peso_precio']);
    }
    $pdf->SimpleTable($header, $data);
...
$sql_leche = "SELECT eh_leche_fecha, eh_leche_peso, eh_leche_precio FROM eh_leche WHERE eh_leche_tagid = ? ORDER BY eh_leche_fecha DESC";
...
if ($result_leche->num_rows > 0) {
    ...
    while ($row = $result_leche->fetch_assoc()) {
        $data[] = array($row['eh_leche_fecha'], $row['eh_leche_peso'], $row['eh_leche_precio']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_concentrado = "SELECT eh_concentrado_fecha, eh_concentrado_racion, eh_concentrado_costo FROM eh_concentrado WHERE eh_concentrado_tagid = ? ORDER BY eh_concentrado_fecha DESC";
...
if ($result_concentrado->num_rows > 0) {
    ...
    while ($row = $result_concentrado->fetch_assoc()) {
        $data[] = array($row['eh_concentrado_fecha'], $row['eh_concentrado_racion'], $row['eh_concentrado_costo']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_salt = "SELECT eh_sal_fecha, eh_sal_racion, eh_sal_costo FROM eh_sal WHERE eh_sal_tagid = ? ORDER BY eh_sal_fecha DESC";
...
if ($result_salt->num_rows > 0) {
    ...
    while ($row = $result_salt->fetch_assoc()) {
        $data[] = array($row['eh_sal_fecha'], $row['eh_sal_racion'], $row['eh_sal_costo']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_molasses = "SELECT eh_melaza_fecha, eh_melaza_racion, eh_melaza_costo FROM eh_melaza WHERE eh_melaza_tagid = ? ORDER BY eh_melaza_fecha DESC";
...
if ($result_molasses->num_rows > 0) {
    ...
    while ($row = $result_molasses->fetch_assoc()) {
        $data[] = array($row['eh_melaza_fecha'], $row['eh_melaza_racion'], $row['eh_melaza_costo']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_aftosa = "SELECT eh_aftosa_fecha, eh_aftosa_producto, eh_aftosa_dosis FROM eh_aftosa WHERE eh_aftosa_tagid = ? ORDER BY eh_aftosa_fecha DESC";
...
if ($result_aftosa->num_rows > 0) {
    ...
    while ($row = $result_aftosa->fetch_assoc()) {
        $data[] = array($row['eh_aftosa_fecha'], $row['eh_aftosa_producto'], $row['eh_aftosa_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_bruc = "SELECT eh_brucelosis_fecha, eh_brucelosis_producto, eh_brucelosis_dosis FROM eh_brucelosis WHERE eh_brucelosis_tagid = ? ORDER BY eh_brucelosis_fecha DESC";
...
if ($result_bruc->num_rows > 0) {
    ...
    while ($row = $result_bruc->fetch_assoc()) {
        $data[] = array($row['eh_brucelosis_fecha'], $row['eh_brucelosis_producto'], $row['eh_brucelosis_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_carbunco = "SELECT eh_carbunco_fecha, eh_carbunco_producto, eh_carbunco_dosis FROM eh_carbunco WHERE eh_carbunco_tagid = ? ORDER BY eh_carbunco_fecha DESC";
...
if ($result_carbunco->num_rows > 0) {
    ...
    while ($row = $result_carbunco->fetch_assoc()) {
        $data[] = array($row['eh_carbunco_fecha'], $row['eh_carbunco_producto'], $row['eh_carbunco_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_ibr = "SELECT eh_ibr_fecha, eh_ibr_producto, eh_ibr_dosis FROM eh_ibr WHERE eh_ibr_tagid = ? ORDER BY eh_ibr_fecha DESC";
...
if ($result_ibr->num_rows > 0) {
    ...
    while ($row = $result_ibr->fetch_assoc()) {
        $data[] = array($row['eh_ibr_fecha'], $row['eh_ibr_producto'], $row['eh_ibr_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_cbr = "SELECT eh_cbr_fecha, eh_cbr_producto, eh_cbr_dosis FROM eh_cbr WHERE eh_cbr_tagid = ? ORDER BY eh_cbr_fecha DESC";
...
if ($result_cbr->num_rows > 0) {
    ...
    while ($row = $result_cbr->fetch_assoc()) {
        $data[] = array($row['eh_cbr_fecha'], $row['eh_cbr_producto'], $row['eh_cbr_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_para = "SELECT eh_parasitos_fecha, eh_parasitos_producto, eh_parasitos_dosis FROM eh_parasitos WHERE eh_parasitos_tagid = ? ORDER BY eh_parasitos_fecha DESC";
...
if ($result_para->num_rows > 0) {
    ...
    while ($row = $result_para->fetch_assoc()) {
        $data[] = array($row['eh_parasitos_fecha'], $row['eh_parasitos_producto'], $row['eh_parasitos_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_tick = "SELECT eh_garrapatas_fecha, eh_garrapatas_producto, eh_garrapatas_dosis FROM eh_garrapatas WHERE eh_garrapatas_tagid = ? ORDER BY eh_garrapatas_fecha DESC";
...
if ($result_tick->num_rows > 0) {
    ...
    while ($row = $result_tick->fetch_assoc()) {
        $data[] = array($row['eh_garrapatas_fecha'], $row['eh_garrapatas_producto'], $row['eh_garrapatas_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_mastitis = "SELECT eh_mastitis_fecha, eh_mastitis_producto, eh_mastitis_dosis FROM eh_mastitis WHERE eh_mastitis_tagid = ? ORDER BY eh_mastitis_fecha DESC";
...
if ($result_mastitis->num_rows > 0) {
    ...
    while ($row = $result_mastitis->fetch_assoc()) {
        $data[] = array($row['eh_mastitis_fecha'], $row['eh_mastitis_producto'], $row['eh_mastitis_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_ins = "SELECT eh_inseminacion_fecha, eh_inseminacion_numero FROM eh_inseminacion WHERE eh_inseminacion_tagid = ? ORDER BY eh_inseminacion_fecha DESC";
...
if ($result_ins->num_rows > 0) {
    ...
    while ($row = $result_ins->fetch_assoc()) {
        $data[] = array($row['eh_inseminacion_fecha'], $row['eh_inseminacion_numero']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_preg = "SELECT eh_gestacion_fecha, eh_gestacion_numero FROM eh_gestacion WHERE eh_gestacion_tagid = ? ORDER BY eh_gestacion_fecha DESC";
...
if ($result_preg->num_rows > 0) {
    ...
    while ($row = $result_preg->fetch_assoc()) {
        $data[] = array($row['eh_gestacion_fecha'], $row['eh_gestacion_numero']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_birth = "SELECT eh_parto_fecha, eh_parto_numero FROM eh_parto WHERE eh_parto_tagid = ? ORDER BY eh_parto_fecha DESC";
...
if ($result_birth->num_rows > 0) {
    ...
    while ($row = $result_birth->fetch_assoc()) {
        $data[] = array($row['eh_parto_fecha'], $row['eh_parto_numero']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_extended_period = "WITH LastBirth AS (
    SELECT 
        eh_parto_tagid,
        MAX(eh_parto_fecha) as ultima_fecha_parto,
...
LEFT JOIN eh_parto p ON v.tagid = p.eh_parto_tagid 
    AND p.eh_parto_fecha > lb.ultima_fecha_parto
WHERE v.genero = 'Hembra' 
    AND v.estatus = 'Activo'
...
$sql_milk_monthly = "WITH MonthlyMilk AS (
    SELECT 
        DATE_FORMAT(eh_leche_fecha, '%Y-%m-01') as primer_dia_mes,
        eh_leche_tagid,
        AVG(eh_leche_peso) as produccion_diaria_promedio,
        AVG(eh_leche_precio) as precio_promedio
    FROM eh_leche
    GROUP BY DATE_FORMAT(eh_leche_fecha, '%Y-%m-01'), eh_leche_tagid
),
MonthlyStats AS (
    SELECT 
        primer_dia_mes as mes,
        COUNT(DISTINCT eh_leche_tagid) as total_vacas,
...
$sql_feed_monthly = "WITH MonthlyFeed AS (
    SELECT 
        DATE_FORMAT(eh_concentrado_fecha, '%Y-%m-01') as primer_dia_mes,
        eh_concentrado_tagid,
        AVG(eh_concentrado_racion) as consumo_diario_promedio,
        AVG(eh_concentrado_costo) as costo_promedio
    FROM eh_concentrado
    GROUP BY DATE_FORMAT(eh_concentrado_fecha, '%Y-%m-01'), eh_concentrado_tagid
),
MonthlyStats AS (
    SELECT 
        primer_dia_mes as mes,
...
$sql_molasses_monthly = "WITH MonthlyMolasses AS (
    SELECT 
        DATE_FORMAT(eh_melaza_fecha, '%Y-%m-01') as primer_dia_mes,
        eh_melaza_tagid,
        AVG(eh_melaza_racion) as consumo_diario_promedio,
        AVG(eh_melaza_costo) as costo_promedio
    FROM eh_melaza
    GROUP BY DATE_FORMAT(eh_melaza_fecha, '%Y-%m-01'), eh_melaza_tagid
),
MonthlyStats AS (
    SELECT 
        primer_dia_mes as mes,
...
$sql_salt_monthly = "WITH MonthlySalt AS (
    SELECT 
        DATE_FORMAT(eh_sal_fecha, '%Y-%m-01') as primer_dia_mes,
        eh_sal_tagid,
        AVG(eh_sal_racion) as consumo_diario_promedio,
        AVG(eh_sal_costo) as costo_promedio
    FROM eh_sal
    GROUP BY DATE_FORMAT(eh_sal_fecha, '%Y-%m-01'), eh_sal_tagid
),
MonthlyStats AS (
    SELECT 
        primer_dia_mes as mes,
...
VaccinationCounts AS (
    SELECT 
        ...
        (SELECT COUNT(DISTINCT eh_garrapatas_tagid) FROM eh_garrapatas) as garrapatas_count,
        ...
        (SELECT COALESCE(SUM(eh_garrapatas_costo * eh_garrapatas_dosis), 0) FROM eh_garrapatas) as garrapatas_cost,
...
try {
    ...
    header('Location: equino_share.php?file=' . urlencode($filename) . '&tagid=' . urlencode($tagid));
    exit;
} catch (Exception $e) {
    // Log error

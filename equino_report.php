...
// PDO connection already has UTF-8 charset set

// Log successful connection
error_log("Database connection established successfully");

// Fetch animal basic info
$sql_animal = "SELECT * FROM equino WHERE tagid = ?";
$stmt_animal = $conn->prepare($sql_animal);
if (!$stmt_animal) {
    error_log('Failed to prepare animal query: ' . implode(', ', $conn->errorInfo()));
    $isAjax = !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && 
               strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest';
    
    if ($isAjax) {
        header('Content-Type: application/json');
        echo json_encode(['success' => false, 'message' => 'Failed to prepare animal query: ' . implode(', ', $conn->errorInfo())]);
        exit;
    } else {
        die('Failed to prepare animal query: ' . implode(', ', $conn->errorInfo()));
    }
}

$stmt_animal->execute([$tagid]);

if ($stmt_animal->rowCount() === 0) {
...
$sql_weight = "SELECT eh_peso_tagid, eh_peso_fecha, eh_peso_animal, eh_peso_precio FROM eh_peso WHERE eh_peso_tagid = ? ORDER BY eh_peso_fecha DESC";
$stmt_weight = $conn->prepare($sql_weight);
$stmt_weight->execute([$tagid]);
$result_weight = $stmt_weight;

if ($result_weight->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha', 'Peso (kg)', 'Precio ($/Kg)');
    $data = array();
    while ($row = $result_weight->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_peso_tagid'], $row['eh_peso_fecha'], $row['eh_peso_animal'], $row['eh_peso_precio']);
    }
    $pdf->SimpleTable($header, $data);
...
$sql_concentrado = "SELECT eh_concentrado_tagid, eh_concentrado_fecha_inicio, eh_concentrado_fecha_fin, eh_concentrado_racion, eh_concentrado_costo FROM eh_concentrado WHERE eh_concentrado_tagid = ? ORDER BY eh_concentrado_fecha_inicio DESC";
$stmt_concentrado = $conn->prepare($sql_concentrado);
$stmt_concentrado->execute([$tagid]);
$result_concentrado = $stmt_concentrado;

if ($result_concentrado->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha Inicio', 'Fecha Fin', 'Consumo Concentrado Peso (kg)', 'Precio ($/Kg)');
    $data = array();
    while ($row = $result_concentrado->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_concentrado_tagid'], $row['eh_concentrado_fecha_inicio'], $row['eh_concentrado_fecha_fin'], $row['eh_concentrado_racion'], $row['eh_concentrado_costo']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_salt = "SELECT eh_sal_tagid, eh_sal_fecha_inicio, eh_sal_fecha_fin, eh_sal_racion, eh_sal_costo FROM eh_sal WHERE eh_sal_tagid = ? ORDER BY eh_sal_fecha_inicio DESC";
$stmt_salt = $conn->prepare($sql_salt);
$stmt_salt->execute([$tagid]);
$result_salt = $stmt_salt;

if ($result_salt->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha Inicio', 'Fecha Fin', 'Racion (Kg/dia)', 'Costo ($/Kg)');
    $data = array();
    while ($row = $result_salt->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_sal_tagid'], $row['eh_sal_fecha_inicio'], $row['eh_sal_fecha_fin'], $row['eh_sal_racion'], $row['eh_sal_costo']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_molasses = "SELECT eh_melaza_tagid, eh_melaza_fecha_inicio, eh_melaza_fecha_fin, eh_melaza_racion, eh_melaza_costo FROM eh_melaza WHERE eh_melaza_tagid = ? ORDER BY eh_melaza_fecha_inicio DESC";
$stmt_molasses = $conn->prepare($sql_molasses);
$stmt_molasses->execute([$tagid]);
$result_molasses = $stmt_molasses;

if ($result_molasses->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha Inicio', 'Fecha Fin', 'Racion (Kg/dia)', 'Costo ($/Kg)');
    $data = array();
    while ($row = $result_molasses->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_melaza_tagid'], $row['eh_melaza_fecha_inicio'], $row['eh_melaza_fecha_fin'], $row['eh_melaza_racion'], $row['eh_melaza_costo']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_aftosa = "SELECT eh_aftosa_tagid, eh_aftosa_fecha, eh_aftosa_producto, eh_aftosa_dosis FROM eh_aftosa WHERE eh_aftosa_tagid = ? ORDER BY eh_aftosa_fecha DESC";
$stmt_aftosa = $conn->prepare($sql_aftosa);
$stmt_aftosa->execute([$tagid]);
$result_aftosa = $stmt_aftosa;

if ($result_aftosa->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha', 'Producto', 'Dosis (ml)');
    $data = array();
    while ($row = $result_aftosa->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_aftosa_tagid'], $row['eh_aftosa_fecha'], $row['eh_aftosa_producto'], $row['eh_aftosa_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_bruc = "SELECT eh_brucelosis_tagid, eh_brucelosis_fecha, eh_brucelosis_producto, eh_brucelosis_dosis FROM eh_brucelosis WHERE eh_brucelosis_tagid = ? ORDER BY eh_brucelosis_fecha DESC";
$stmt_bruc = $conn->prepare($sql_bruc);
$stmt_bruc->execute([$tagid]);
$result_bruc = $stmt_bruc;

if ($result_bruc->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha', 'Producto', 'Dosis (ml)');
    $data = array();
    while ($row = $result_bruc->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_brucelosis_tagid'], $row['eh_brucelosis_fecha'], $row['eh_brucelosis_producto'], $row['eh_brucelosis_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_carbunco = "SELECT eh_carbunco_tagid, eh_carbunco_fecha, eh_carbunco_producto, eh_carbunco_dosis FROM eh_carbunco WHERE eh_carbunco_tagid = ? ORDER BY eh_carbunco_fecha DESC";
$stmt_carbunco = $conn->prepare($sql_carbunco);
$stmt_carbunco->execute([$tagid]);
$result_carbunco = $stmt_carbunco;

if ($result_carbunco->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha', 'Producto', 'Dosis (ml)');
    $data = array();
    while ($row = $result_carbunco->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_carbunco_tagid'], $row['eh_carbunco_fecha'], $row['eh_carbunco_producto'], $row['eh_carbunco_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_ibr = "SELECT eh_ibr_tagid, eh_ibr_fecha, eh_ibr_producto, eh_ibr_dosis FROM eh_ibr WHERE eh_ibr_tagid = ? ORDER BY eh_ibr_fecha DESC";
$stmt_ibr = $conn->prepare($sql_ibr);
$stmt_ibr->execute([$tagid]);
$result_ibr = $stmt_ibr;

if ($result_ibr->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha', 'Producto', 'Dosis (ml)');
    $data = array();
    while ($row = $result_ibr->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_ibr_tagid'], $row['eh_ibr_fecha'], $row['eh_ibr_producto'], $row['eh_ibr_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_cbr = "SELECT eh_cbr_tagid, eh_cbr_fecha, eh_cbr_producto, eh_cbr_dosis FROM eh_cbr WHERE eh_cbr_tagid = ? ORDER BY eh_cbr_fecha DESC";
$stmt_cbr = $conn->prepare($sql_cbr);
$stmt_cbr->execute([$tagid]);
$result_cbr = $stmt_cbr;

if ($result_cbr->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha', 'Producto', 'Dosis (ml)');
    $data = array();
    while ($row = $result_cbr->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_cbr_tagid'], $row['eh_cbr_fecha'], $row['eh_cbr_producto'], $row['eh_cbr_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_para = "SELECT eh_parasitos_tagid, eh_parasitos_fecha, eh_parasitos_producto, eh_parasitos_dosis FROM eh_parasitos WHERE eh_parasitos_tagid = ? ORDER BY eh_parasitos_fecha DESC";
$stmt_para = $conn->prepare($sql_para);
$stmt_para->execute([$tagid]);
$result_para = $stmt_para;

if ($result_para->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha', 'Producto', 'Dosis (ml)');
    $data = array();
    while ($row = $result_para->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_parasitos_tagid'], $row['eh_parasitos_fecha'], $row['eh_parasitos_producto'], $row['eh_parasitos_dosis']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_ins = "SELECT eh_inseminacion_tagid, eh_inseminacion_fecha, eh_inseminacion_numero FROM eh_inseminacion WHERE eh_inseminacion_tagid = ? ORDER BY eh_inseminacion_fecha DESC";
$stmt_ins = $conn->prepare($sql_ins);
$stmt_ins->execute([$tagid]);
$result_ins = $stmt_ins;

if ($result_ins->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha', 'Inseminacion Nro.');
    $data = array();
    while ($row = $result_ins->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_inseminacion_tagid'], $row['eh_inseminacion_fecha'], $row['eh_inseminacion_numero']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_preg = "SELECT eh_gestacion_tagid, eh_gestacion_fecha, eh_gestacion_numero FROM eh_gestacion WHERE eh_gestacion_tagid = ? ORDER BY eh_gestacion_fecha DESC";
$stmt_preg = $conn->prepare($sql_preg);
$stmt_preg->execute([$tagid]);
$result_preg = $stmt_preg;

if ($result_preg->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha', 'Gestacion Nro.');
    $data = array();
    while ($row = $result_preg->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_gestacion_tagid'], $row['eh_gestacion_fecha'], $row['eh_gestacion_numero']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_birth = "SELECT eh_parto_tagid, eh_parto_fecha, eh_parto_numero FROM eh_parto WHERE eh_parto_tagid = ? ORDER BY eh_parto_fecha DESC";
$stmt_birth = $conn->prepare($sql_birth);
$stmt_birth->execute([$tagid]);
$result_birth = $stmt_birth;

if ($result_birth->rowCount() > 0) {
    $header = array('Tag ID', 'Fecha', 'Parto Nro.');
    $data = array();
    while ($row = $result_birth->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array($row['eh_parto_tagid'], $row['eh_parto_fecha'], $row['eh_parto_numero']);
    }
    $pdf->SimpleTable($header, $data);
} else {
    $pdf->SetFont('Arial', 'I', 10);
...
$sql_breeds = "SELECT 
    ...
    ROUND((COUNT(*) * 100.0) / (SELECT COUNT(*) FROM equino WHERE estatus = 'Activo'), 1) as porcentaje
FROM equino 
WHERE estatus = 'Activo'
GROUP BY raza
ORDER BY total_animales DESC";

$result_breeds = $conn->prepare($sql_breeds);
$result_breeds->execute();

if ($result_breeds->rowCount() > 0) {
    $header = array('Raza', 'Total Animales', 'Porcentaje (%)');
    $data = array();
    $total_animals = 0;
    
    while ($row = $result_breeds->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array(
...
$result_groups = $conn->prepare($sql_groups);
$result_groups->execute();

if ($result_groups->rowCount() > 0) {
    $header = array('Grupo', 'Total Animales', 'Porcentaje (%)');
    $data = array();
    $total_animals = 0;
    
    while ($row = $result_groups->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array(
            $row['grupo'],
            $row['total_animales'],
            number_format($row['porcentaje'], 1)
...
$sql_monthly = "WITH MonthlyWeights AS (
    SELECT 
        DATE_FORMAT(eh_peso_fecha, '%Y-%m-01') as primer_dia_mes,
        eh_peso_tagid,
        AVG(eh_peso_animal) as peso_promedio_animal
    FROM eh_peso 
    GROUP BY DATE_FORMAT(eh_peso_fecha, '%Y-%m-01'), eh_peso_tagid
)
SELECT 
    primer_dia_mes as mes,
    COUNT(DISTINCT eh_peso_tagid) as total_animales,
...
$result_monthly = $conn->prepare($sql_monthly);
$result_monthly->execute();

if ($result_monthly->rowCount() > 0) {
    ...
    while ($row = $result_monthly->fetch(PDO::FETCH_ASSOC)) {
...
$sql_feed_monthly = "WITH MonthlyFeed AS (
    SELECT 
        DATE_FORMAT(eh_concentrado_fecha_inicio, '%Y-%m-01') as primer_dia_mes,
        eh_concentrado_tagid,
        AVG(eh_concentrado_racion) as consumo_diario_promedio,
        AVG(eh_concentrado_costo) as costo_promedio
    FROM eh_concentrado
    GROUP BY DATE_FORMAT(eh_concentrado_fecha_inicio, '%Y-%m-01'), eh_concentrado_tagid
),
MonthlyStats AS (
    SELECT 
        primer_dia_mes as mes,
        COUNT(DISTINCT eh_concentrado_tagid) as total_animales,
...
$result_feed_monthly = $conn->prepare($sql_feed_monthly);
$result_feed_monthly->execute();

if ($result_feed_monthly->rowCount() > 0) {
    ...
    while ($row = $result_feed_monthly->fetch(PDO::FETCH_ASSOC)) {
        // Format the month to Spanish format
        $date = DateTime::createFromFormat('Y-m-d', $row['mes']);
        $mes_espanol = strftime('%B %Y', $date->getTimestamp());
        $mes_espanol = ucfirst(mb_strtolower($mes_espanol, 'UTF-8'));
...
MonthlyWeights AS (
    SELECT 
        eh_peso_tagid,
        DATE_FORMAT(eh_peso_fecha, '%Y-%m-01') as mes,
        AVG(eh_peso_animal) as peso_promedio
    FROM eh_peso
    GROUP BY eh_peso_tagid, DATE_FORMAT(eh_peso_fecha, '%Y-%m-01')
),
WeightChanges AS (
    SELECT 
        w1.eh_peso_tagid,
    ...
    JOIN MonthlyWeights w2 ON w1.eh_peso_tagid = w2.eh_peso_tagid
        AND w1.mes < w2.mes
        AND NOT EXISTS (
            SELECT 1 FROM MonthlyWeights w3
            WHERE w3.eh_peso_tagid = w1.eh_peso_tagid
...
TotalFeed AS (
    SELECT 
        eh_concentrado_tagid,
        DATE_FORMAT(eh_concentrado_fecha_inicio, '%Y-%m-01') as mes,
        SUM(eh_concentrado_racion) as consumo_total
    FROM eh_concentrado
    GROUP BY eh_concentrado_tagid, DATE_FORMAT(eh_concentrado_fecha_inicio, '%Y-%m-01')
),
FCRCalculation AS (
    SELECT 
        wc.eh_peso_tagid,
    ...
    JOIN AllAnimals a ON wc.eh_peso_tagid = a.tagid
    LEFT JOIN TotalFeed tf ON wc.eh_peso_tagid = tf.eh_concentrado_tagid
        AND tf.mes >= wc.mes_inicial AND tf.mes <= wc.mes_final
    GROUP BY wc.eh_peso_tagid, a.nombre, a.genero, a.etapa, wc.mes_inicial, wc.mes_final, 
...
SELECT 
    ...
    COUNT(DISTINCT eh_peso_tagid) as animales_con_ica,
...
$result_fcr = $conn->prepare($sql_fcr);
$result_fcr->execute();

if ($result_fcr->rowCount() > 0) {
    $fcr_data = $result_fcr->fetch(PDO::FETCH_ASSOC);
...
// Using date ranges (fecha_inicio to fecha_fin) to calculate consumption per month
$sql_molasses_monthly = "WITH MonthlyMolasses AS (
    SELECT 
        DATE_FORMAT(eh_melaza_fecha_inicio, '%Y-%m-01') as primer_dia_mes,
        eh_melaza_tagid,
        AVG(eh_melaza_racion) as consumo_diario_promedio,
        AVG(eh_melaza_costo) as costo_promedio
    FROM eh_melaza
    WHERE eh_melaza_fecha_inicio IS NOT NULL
    GROUP BY DATE_FORMAT(eh_melaza_fecha_inicio, '%Y-%m-01'), eh_melaza_tagid
),
MonthlyStats AS (
    SELECT 
        primer_dia_mes as mes,
        COUNT(DISTINCT eh_melaza_tagid) as total_animales,
        ROUND(SUM(consumo_diario_promedio), 2) as consumo_diario_total,
        ROUND(AVG(consumo_diario_promedio), 2) as promedio_diario_por_animal,
        ROUND(AVG(costo_promedio), 2) as costo_promedio_mes
...
$result_molasses_monthly = $conn->prepare($sql_molasses_monthly);
$result_molasses_monthly->execute();

if ($result_molasses_monthly->rowCount() > 0) {
    $header = array('Mes', '# Animales', 'C. Diario', 'C. Mensual', 'Diario x Animal', 'Mensual x Animal', 'Precio Prom.', 'Total');
    $data = array();
    ...
    while ($row = $result_molasses_monthly->fetch(PDO::FETCH_ASSOC)) {
        // Format the month to Spanish format
        $date = DateTime::createFromFormat('Y-m-d', $row['mes']);
        $mes_espanol = strftime('%B %Y', $date->getTimestamp());
        $mes_espanol = ucfirst(mb_strtolower($mes_espanol, 'UTF-8'));
...
$sql_salt_monthly = "WITH MonthlySalt AS (
    SELECT 
        DATE_FORMAT(eh_sal_fecha_inicio, '%Y-%m-01') as primer_dia_mes,
        eh_sal_tagid,
        AVG(eh_sal_racion) as consumo_diario_promedio,
        AVG(eh_sal_costo) as costo_promedio
    FROM eh_sal
    WHERE eh_sal_fecha_inicio IS NOT NULL
    GROUP BY DATE_FORMAT(eh_sal_fecha_inicio, '%Y-%m-01'), eh_sal_tagid
),
MonthlyStats AS (
    SELECT 
        primer_dia_mes as mes,
        COUNT(DISTINCT eh_sal_tagid) as total_animales,
        ROUND(SUM(consumo_diario_promedio), 2) as consumo_diario_total,
        ROUND(AVG(consumo_diario_promedio), 2) as promedio_diario_por_animal,
        ROUND(AVG(costo_promedio), 2) as costo_promedio_mes
    FROM MonthlySalt
...
$result_salt_monthly = $conn->prepare($sql_salt_monthly);
$result_salt_monthly->execute();

if ($result_salt_monthly->rowCount() > 0) {
    $header = array('Mes', '# Animales', 'C. Diario', 'C. Mensual', 'Diario x Animal', 'Mensual x Animal', 'Precio Prom.', 'Total');
    $data = array();
    ...
    while ($row = $result_salt_monthly->fetch(PDO::FETCH_ASSOC)) {
        // Format the month to Spanish format
        $date = DateTime::createFromFormat('Y-m-d', $row['mes']);
        $mes_espanol = strftime('%B %Y', $date->getTimestamp());
        $mes_espanol = ucfirst(mb_strtolower($mes_espanol, 'UTF-8'));
...
$sql_vacc_summary = "
WITH AllAnimals AS (
    SELECT DISTINCT tagid FROM equino WHERE estatus = 'Activo'
),
VaccinationCounts AS (
    SELECT 
        (SELECT COUNT(*) FROM AllAnimals) as total_animals,
        (SELECT COUNT(DISTINCT eh_aftosa_tagid) FROM eh_aftosa) as aftosa_count,
        (SELECT COUNT(DISTINCT eh_brucelosis_tagid) FROM eh_brucelosis) as brucelosis_count,
        (SELECT COUNT(DISTINCT eh_cbr_tagid) FROM eh_cbr) as cbr_count,
        (SELECT COUNT(DISTINCT eh_ibr_tagid) FROM eh_ibr) as ibr_count,
        (SELECT COUNT(DISTINCT eh_carbunco_tagid) FROM eh_carbunco) as carbunco_count,
        (SELECT COUNT(DISTINCT eh_parasitos_tagid) FROM eh_parasitos) as parasitos_count,
        (SELECT COALESCE(SUM(eh_aftosa_costo * eh_aftosa_dosis), 0) FROM eh_aftosa) as aftosa_cost,
        (SELECT COALESCE(SUM(eh_brucelosis_costo * eh_brucelosis_dosis), 0) FROM eh_brucelosis) as brucelosis_cost,
        (SELECT COALESCE(SUM(eh_cbr_costo * eh_cbr_dosis), 0) FROM eh_cbr) as cbr_cost,
        (SELECT COALESCE(SUM(eh_ibr_costo * eh_ibr_dosis), 0) FROM eh_ibr) as ibr_cost,
        (SELECT COALESCE(SUM(eh_carbunco_costo * eh_carbunco_dosis), 0) FROM eh_carbunco) as carbunco_cost,
        (SELECT COALESCE(SUM(eh_parasitos_costo * eh_parasitos_dosis), 0) FROM eh_parasitos) as parasitos_cost
)
SELECT 
    total_animals,
    aftosa_count,
...
$result_vacc = $conn->prepare($sql_vacc_summary);
$result_vacc->execute();
$vacc_data = $result_vacc->fetch(PDO::FETCH_ASSOC);
...
$sql_preg_duration = "SELECT 
    g.eh_gestacion_tagid,
    v.nombre,
    g.eh_gestacion_numero,
    g.eh_gestacion_fecha,
    p.eh_parto_fecha,
    CASE 
        WHEN p.eh_parto_fecha IS NOT NULL THEN 'Completada'
        ELSE 'En Curso'
    END as estado,
    DATEDIFF(COALESCE(p.eh_parto_fecha, CURDATE()), g.eh_gestacion_fecha) as dias_gestacion
FROM eh_gestacion g
LEFT JOIN eh_parto p ON g.eh_gestacion_tagid = p.eh_parto_tagid 
    AND g.eh_gestacion_numero = p.eh_parto_numero
LEFT JOIN equino v ON g.eh_gestacion_tagid = v.tagid
ORDER BY g.eh_gestacion_tagid, g.eh_gestacion_fecha DESC";

$stmt_preg_duration = $conn->prepare($sql_preg_duration);
$stmt_preg_duration->execute();
$result_preg_duration = $stmt_preg_duration;

if ($result_preg_duration->rowCount() > 0) {
    ...
    while ($row = $result_preg_duration->fetch(PDO::FETCH_ASSOC)) {
        $parto_fecha = $row['eh_parto_fecha'] ? $row['eh_parto_fecha'] : 'En Curso';
        $data[] = array(
            $row['eh_gestacion_tagid'],
            $row['nombre'],
            $row['eh_gestacion_numero'],
            $row['eh_gestacion_fecha'],
            $parto_fecha,
            $row['estado'],
            $row['dias_gestacion']
        );
...
$sql_open_days = "WITH OrderedEvents AS (
    SELECT 
        eh_parto_tagid as tagid,
        eh_parto_fecha as fecha,
        eh_parto_numero as numero,
        'Parto' as tipo
    FROM eh_parto
...
LEFT JOIN equino v ON np.tagid = v.tagid
...
$result_open_days = $stmt_open_days;

if ($result_open_days->rowCount() > 0) {
    ...
    while ($row = $result_open_days->fetch(PDO::FETCH_ASSOC)) {
...
FROM equino v
LEFT JOIN eh_gestacion g ON v.tagid = g.eh_gestacion_tagid
WHERE v.genero = 'Hembra' 
    AND g.eh_gestacion_tagid IS NULL
    AND v.estatus = 'Activo'
ORDER BY v.fecha_nacimiento ASC";

$stmt_no_preg = $conn->prepare($sql_no_preg);
$stmt_no_preg->execute();
$result_no_preg = $stmt_no_preg;

if ($result_no_preg->rowCount() > 0) {
    ...
    while ($row = $result_no_preg->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array(
            $row['tagid'],
            $row['nombre'],
...
$sql_extended_period = "WITH LastBirth AS (
    SELECT 
        eh_parto_tagid,
        MAX(eh_parto_fecha) as eh_parto_fecha,
        COUNT(*) as total_partos
    FROM eh_parto
    GROUP BY eh_parto_tagid
)
SELECT 
    v.tagid,
    v.nombre,
    v.etapa,
    lb.eh_parto_fecha,
    DATEDIFF(CURDATE(), lb.eh_parto_fecha) as dias_desde_parto,
    lb.total_partos
FROM equino v
JOIN LastBirth lb ON v.tagid = lb.eh_parto_tagid
LEFT JOIN eh_parto p ON v.tagid = p.eh_parto_tagid 
    AND p.eh_parto_fecha > lb.eh_parto_fecha
WHERE v.genero = 'Hembra' 
    AND v.estatus = 'Activo'
    AND DATEDIFF(CURDATE(), lb.eh_parto_fecha) > 365
    AND p.eh_parto_tagid IS NULL
ORDER BY dias_desde_parto DESC";

$stmt_extended = $conn->prepare($sql_extended_period);
$stmt_extended->execute();
$result_extended = $stmt_extended;

if ($result_extended->rowCount() > 0) {
    ...
    while ($row = $result_extended->fetch(PDO::FETCH_ASSOC)) {
        $data[] = array(
            $row['tagid'],
            $row['nombre'],
            $row['etapa'],
...
try {
    ...
    $share_file = __DIR__ . '/equino_share.php';
    ...
    if ($isAjax) {
        ...
        $redirect_url = 'equino_share.php?file=' . urlencode($filename) . '&tagid=' . urlencode($tagid);

...
<title>Equino Register Carne</title>
<!-- Link to the Favicon -->
                  ...
                  try {
                      // Query to get all Animals and ALL their peso records (if any)
                        $pesoQuery = "
                            SELECT
                                ...
                                p.eh_peso_fecha,
                                p.eh_peso_tagid,         -- Matches equino_tagid if peso exists
                                p.eh_peso_animal,
                                p.eh_peso_precio,
                                -- Calculate total_value only if p.id is not null
                                CASE WHEN p.id IS NOT NULL THEN CAST((p.eh_peso_animal * p.eh_peso_precio) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                equino v
                            LEFT JOIN
                                eh_peso p ON v.tagid = p.eh_peso_tagid -- Join ALL matching peso records
                            ORDER BY
                      ...
                      if (empty($pesosData)) {
                          ...
                          foreach ($pesosData as $row) {
                              ...
                              $pesoFecha = $row['eh_peso_fecha'] ?? null;
                              ...
                              if ($hasPeso) {
                                  ...
                                  echo '        <button class="btn btn-danger btn-sm delete-peso" 
                                                  data-id="'.htmlspecialchars($row['peso_id'] ?? '').'" 
                                                  data-tagid="'.htmlspecialchars($row['eh_peso_tagid'] ?? '').'" -- Pass tagid for context
                                                  title="Eliminar Peso">
                              ...
                              // Column 5: Peso (kg)
                              echo "<td>" . ($hasPeso ? htmlspecialchars($row['eh_peso_animal'] ?? '') : 'N/A') . "</td>";
                              // Column 6: Precio ($/kg)
                              echo "<td>" . ($hasPeso ? htmlspecialchars($row['eh_peso_precio'] ?? '') : 'N/A') . "</td>";

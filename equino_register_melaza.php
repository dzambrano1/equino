...
try {
    ...
    $melazaQuery = "SELECT 
                        ...
                        AND eh_melaza_racion IS NOT NULL 
                        AND eh_melaza_costo IS NOT NULL
    ...
    if (empty($melazaData)) {
        ...
        $melazaQuery = "SELECT 
                                eh_melaza_fecha,
                                eh_melaza_fecha as eh_melaza_fecha_inicio,
                                eh_melaza_fecha as eh_melaza_fecha_fin,
                                eh_melaza_racion,
                                eh_melaza_costo,
                                eh_melaza_tagid
                            FROM eh_melaza 
    ...
    foreach ($melazaData as $melaza) {
        $startDate = new DateTime($melaza['eh_melaza_fecha_inicio']);
        $endDate = new DateTime($melaza['eh_melaza_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $melaza['eh_melaza_racion'] * $melaza['eh_melaza_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing melaza: Start=" . $melaza['eh_melaza_fecha_inicio'] . 
                 ", End=" . $melaza['eh_melaza_fecha_fin'] . 
                 ", Days=" . $totalDays . 
...
<title>equino - Registro de Melaza</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_alimento" name="alimento" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_alimentos = "SELECT DISTINCT ec_melaza_nombre FROM ec_melaza ORDER BY ec_melaza_nombre ASC";
                                        ...
                                        foreach ($alimentos as $alimento_row) {
                                            echo '<option value="' . htmlspecialchars($alimento_row['ec_melaza_nombre']) . '">' . htmlspecialchars($alimento_row['ec_melaza_nombre']) . '</option>';
                                        }
                                    } catch (PDOException $e) {
  ...
  <!-- DataTable for eh_melaza records -->
                  ...
                  try {
                      // Query to get all Animals and ALL their melaza records (if any)
                        $melazaQuery = "
                            SELECT
                                ...
                                c.eh_melaza_fecha_inicio,
                                c.eh_melaza_fecha_fin,
                                c.eh_melaza_tagid,         -- Matches equino_tagid if melaza exists
                                c.eh_melaza_etapa,
                                c.eh_melaza_producto,
                                c.eh_melaza_racion,
                                c.eh_melaza_costo,
                                -- Calculate total_value only if c.id is not null
                                CASE WHEN c.id IS NOT NULL THEN CAST((c.eh_melaza_racion * c.eh_melaza_costo) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                equino v
                            LEFT JOIN
                                eh_melaza c ON v.tagid = c.eh_melaza_tagid -- Join ALL matching melaza records
                            ORDER BY
                      ...
                      if (empty($melazaData)) {
                          ...
                          foreach ($melazaData as $row) {
                              ...
                              $melazaFecha = $row['eh_melaza_fecha_inicio'] ?? null;
                              $melazaFechaInicio = $row['eh_melaza_fecha_inicio'] ?? null;
                              $melazaFechaFin = $row['eh_melaza_fecha_fin'] ?? null;
                              ...
                              if ($hasMelaza) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-melaza" 
                                                  ...
                                                  data-debug="DB: '.htmlspecialchars($row['eh_melaza_producto'] ?? 'NULL').'">
                              ...
                              echo "<td>" . ($hasMelaza ? htmlspecialchars($row['eh_melaza_producto'] ?? '') : 'N/A') . "</td>";
                              // Column 7: Racion (or N/A)
                              echo "<td>" . ($hasMelaza ? htmlspecialchars($row['eh_melaza_racion'] ?? '') : 'N/A') . "</td>";
                              // Column 8: Costo (or N/A)
                              echo "<td>" . ($hasMelaza ? htmlspecialchars($row['eh_melaza_costo'] ?? '') : 'N/A') . "</td>";
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['ec_melaza_nombre']) . '">' . htmlspecialchars($alimento_row['ec_melaza_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {

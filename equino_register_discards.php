...
<title>equino Register Herd</title>
<!-- Link to the Favicon -->
...
<a href="./equino_registros.php" class="back-btn">
...
<div class="container text-center">
  ...
  <!-- DataTable for eh_compra records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="eh_compraTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                    // Query to get all eh_compra records with animal name
                    $compraQuery = "SELECT c.*, v.nombre as animal_nombre
                              FROM eh_compra c 
                              LEFT JOIN equino v ON c.eh_compra_tagid = v.tagid 
                              ORDER BY c.eh_compra_fecha DESC";
                    ...
                    if (empty($compraData)) {
                        ...
                        foreach ($compraData as $row) {
                            ...
                            echo "<td class='text-center'>" . htmlspecialchars($row['eh_compra_fecha'] ?? '') . "</td>";                        
                            echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['eh_compra_tagid'] ?? '') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['eh_compra_precio'] ?? '') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['eh_compra_peso'] ?? '') . "</td>";

                            // Calculate due date and determine status
                            try {
                                $compraDate = new DateTime($row['eh_compra_fecha']);
                                ...
                                error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_compra_fecha']);
                            ...
                            echo '<td class="text-center">
                                <div class="btn-group" role="group">
                                    <button class="btn btn-warning btn-sm edit-compra" 
                                        ...
                                        data-tagid="' . htmlspecialchars($row['eh_compra_tagid'] ?? '') . '"
                                        data-fecha="' . htmlspecialchars($row['eh_compra_fecha'] ?? '') . '"
                                        data-precio="' . htmlspecialchars($row['eh_compra_precio'] ?? '') . '"
                                        data-peso="' . htmlspecialchars($row['eh_compra_peso'] ?? '') . '">
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.eh_compra_tagid && !uniqueTagIds.has(item.eh_compra_tagid)) {
                uniqueTagIds.add(item.eh_compra_tagid);
                animals.push({
                    tagid: item.eh_compra_tagid,
    ...
    function updateChart() {
        ...
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.eh_compra_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.eh_compra_fecha) - new Date(b.eh_compra_fecha));
    ...
    function createCompraChart(data) {
        ...
        var labels = data.map(function(item) {
            ...
            var parts = item.eh_compra_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.eh_compra_fecha;
        });
        
        var precio = data.map(function(item) {
            return item.eh_compra_precio;
        });
...
<div class="container text-center">
  ...
  <!-- DataTable for eh_deceso records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="eh_decesoTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      <th class="text-center">ID</th>
                      <th class="text-center">Fecha</th>
              ...
              <tbody>
                  <?php
                  try {
                    // Query to get all eh_parto records with animal name
                    $decesoQuery = "SELECT c.*, v.nombre as animal_nombre
                              FROM eh_deceso c 
                              LEFT JOIN equino v ON c.eh_deceso_tagid = v.tagid 
                              ORDER BY c.eh_deceso_fecha DESC";
                    ...
                    if (empty($decesoData)) {
                        ...
                        foreach ($decesoData as $row) {
                            echo "<tr>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['id'] ?? '') . "</td>";                        
                            echo "<td class='text-center'>" . htmlspecialchars($row['eh_deceso_fecha'] ?? '') . "</td>";                        
                            echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['eh_deceso_tagid'] ?? '') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['eh_deceso_causa'] ?? '') . "</td>";
                            // Calculate due date and determine status
                            try {
                                $decesoDate = new DateTime($row['eh_deceso_fecha']);
                                ...
                                // Handle invalid date format
                                error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_deceso_fecha']);
                                echo '<td class="text-center"><span class="badge bg-warning">ERROR</span></td>';
                            }
                            ...
                            echo '<td class="text-center">
                                <div class="btn-group" role="group">
                                    <button class="btn btn-warning btn-sm edit-deceso" 
                                        data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                        data-tagid="' . htmlspecialchars($row['eh_deceso_tagid'] ?? '') . '"
                                        data-fecha="' . htmlspecialchars($row['eh_deceso_fecha'] ?? '') . '"
                                        data-peso="' . htmlspecialchars($row['eh_deceso_peso'] ?? '') . '">
                                        <i class="fas fa-edit"></i>
                                    </button>
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.eh_deceso_tagid && !uniqueTagIds.has(item.eh_deceso_tagid)) {
                uniqueTagIds.add(item.eh_deceso_tagid);
                animals.push({
                    tagid: item.eh_deceso_tagid,
                    nombre: item.animal_nombre || 'Sin nombre'
                });
            }
        });
    ...
    function updateChart() {
        ...
        // Filter by animal if not "all"
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.eh_deceso_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.eh_deceso_fecha) - new Date(b.eh_deceso_fecha));
    ...
    function createDecesoChart(data) {
        ...
        var labels = data.map(function(item) {
            // Format the date for display
            var parts = item.eh_deceso_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.eh_deceso_fecha;
        });
        
        var causa = data.map(function(item) {
            return item.eh_deceso_causa;
        });
...
<div class="container text-center">
  ...
  <!-- DataTable for eh_descarte records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="eh_descarteTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      <th class="text-center">ID</th>
                      <th class="text-center">Fecha</th>
              ...
              <tbody>
                  ...
                  try {
                    ...
                    $descarteQuery = "SELECT c.*, v.nombre as animal_nombre
                              FROM eh_descarte c 
                              LEFT JOIN equino v ON c.eh_descarte_tagid = v.tagid 
                              ORDER BY c.eh_descarte_fecha DESC";
                    ...
                    if (empty($descarteData)) {
                        ...
                        foreach ($descarteData as $row) {
                            echo "<tr>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['id'] ?? '') . "</td>";                        
                            echo "<td class='text-center'>" . htmlspecialchars($row['eh_descarte_fecha'] ?? '') . "</td>";                        
                            echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['eh_descarte_tagid'] ?? '') . "</td>";
                            echo "<td class='text-center'>" . htmlspecialchars($row['eh_descarte_peso'] ?? '') . "</td>";
                            
                            // Calculate due date and determine status
                            try {
                                $descarteDate = new DateTime($row['eh_descarte_fecha']);
                                ...
                                // Handle invalid date format
                                error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_descarte_fecha']);
                                echo '<td class="text-center"><span class="badge bg-warning">ERROR</span></td>';
                            }
                            ...
                            echo '<td class="text-center">
                                <div class="btn-group" role="group">
                                    <button class="btn btn-warning btn-sm edit-descarte" 
                                        data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                        data-tagid="' . htmlspecialchars($row['eh_descarte_tagid'] ?? '') . '"
                                        data-fecha="' . htmlspecialchars($row['eh_descarte_fecha'] ?? '') . '"
                                        data-peso="' . htmlspecialchars($row['eh_descarte_peso'] ?? '') . '">
                                        <i class="fas fa-edit"></i>
                                    </button>
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.eh_descarte_tagid && !uniqueTagIds.has(item.eh_descarte_tagid)) {
                uniqueTagIds.add(item.eh_descarte_tagid);
                animals.push({
                    tagid: item.eh_descarte_tagid,
                    nombre: item.animal_nombre || 'Sin nombre'
                });
            }
        });
    ...
    function updateChart() {
        ...
        // Filter by animal if not "all"
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.eh_descarte_tagid === selectedAnimal);
        }
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.eh_descarte_fecha) - new Date(b.eh_descarte_fecha));
    ...
    function createDescarteChart(data) {
        ...
        var labels = data.map(function(item) {
            // Format the date for display
            var parts = item.eh_descarte_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.eh_descarte_fecha;
        });
        
        var causa = data.map(function(item) {
            return item.eh_descarte_causa;
        });
...
<div class="container text-center">
  ...
  <!-- DataTable for eh_venta records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="eh_ventaTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
                      <th class="text-center">ID</th>
                      <th class="text-center">Fecha</th>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all eh_venta records with animal name
                      $ventaQuery = "SELECT v.*, v.nombre as animal_nombre,
                                ve.eh_venta_peso as eh_venta_peso,
                                ve.eh_venta_precio as eh_venta_precio,
                                ve.eh_venta_fecha as eh_venta_fecha,
                                ve.eh_venta_tagid as eh_venta_tagid
                                FROM eh_venta ve 
                                JOIN equino v ON v.tagid = ve.eh_venta_tagid 
                                ORDER BY ve.eh_venta_fecha DESC";
                      ...
                      if (empty($ventaData)) {
                          ...
                          foreach ($ventaData as $row) {
                              echo "<tr>";
                              echo "<td class='text-center'>" . htmlspecialchars($row['id'] ?? '') . "</td>";                        
                              echo "<td class='text-center'>" . htmlspecialchars($row['eh_venta_fecha'] ?? '') . "</td>";                        
                              echo "<td class='text-center'>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                              echo "<td class='text-center'>" . htmlspecialchars($row['eh_venta_tagid'] ?? '') . "</td>";
                              echo "<td class='text-center'>" . htmlspecialchars($row['eh_venta_peso'] ?? '') . "</td>";
                              echo "<td class='text-center'>" . htmlspecialchars($row['eh_venta_precio'] ?? '') . "</td>";

                              // Calculate due date and determine status
                              try {
                                  $ventaDate = new DateTime($row['eh_venta_fecha']);
                                  ...
                                  // Handle invalid date format
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_venta_fecha']);
                                  echo '<td class="text-center"><span class="badge bg-warning">ERROR</span></td>';
                              }
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-venta" 
                                              data-id="' . htmlspecialchars($row['id'] ?? '') . '"
                                              data-tagid="' . htmlspecialchars($row['eh_venta_tagid'] ?? '') . '"
                                              data-fecha="' . htmlspecialchars($row['eh_venta_fecha'] ?? '') . '"
                                              data-peso="' . htmlspecialchars($row['eh_venta_peso'] ?? '') . '"
                                          data-precio="' . htmlspecialchars($row['eh_venta_precio'] ?? '') . '">
                                          <i class="fas fa-edit"></i>
                                      </button>
...
$(document).ready(function() {
    ...
    function populateAnimalFilter(data) {
        ...
        data.forEach(item => {
            if (item.eh_venta_tagid && !uniqueTagIds.has(item.eh_venta_tagid)) {
                uniqueTagIds.add(item.eh_venta_tagid);
                animals.push({
                    tagid: item.eh_venta_tagid,
                    nombre: item.animal_nombre || 'Sin nombre'
                });
            }
        });
    ...
    function updateChart() {
        ...
        // Filter by animal if not "all"
        if (selectedAnimal !== 'all') {
            filteredData = filteredData.filter(item => item.eh_venta_tagid === selectedAnimal);
        }   
        
        // Sort data by date
        filteredData.sort((a, b) => new Date(a.eh_venta_fecha) - new Date(b.eh_venta_fecha));
    ...
    function createVentaChart(data) {
        ...
        var labels = data.map(function(item) {
            // Format the date for display
            var parts = item.eh_venta_fecha.split('-');
            if (parts.length === 3) {
                return parts[2] + '/' + parts[1] + '/' + parts[0];
            }
            return item.eh_venta_fecha;
        });
        
        var precio = data.map(function(item) {
            return item.eh_venta_precio;
        });

...
<title>equino Register Molasses</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  <div class="container mt-3 mb-4 text-center">
    <button type="button" class="btn btn-add-animal" data-bs-toggle="modal" data-bs-target="#newMolassesModal">
  ...
  <div class="modal fade" id="newMolassesModal" tabindex="-1" aria-labelledby="newMolassesModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="newMolassesModalLabel">
                    ...
                    <div class="mb-4">                        
                        <div class="input-group">
                            ...
                            <i class="fa-solid fa-layer-group"></i>
                                ...
                                <input type="text" class="form-control" id="new_etapa" name="etapa" required>
                            </span>                                
                        </div>
                    </div>
                    <div class="mb-4">                        
                        <div class="input-group">
                            ...
                            <i class="fa-solid fa-plate-wheat"></i>
                                <label for="new_producto" class="form-label">Producto</label>
                                <input type="text" class="form-control" id="new_producto" name="producto" required>
                            </span>
                        </div>
                    </div>
                    <div class="mb-4">                        
  ...
  <!-- DataTable for eh_molasses records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="eh_molassesTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all eh_molasses records with animal name
                      $molassesQuery = "SELECT c.*, v.nombre as animal_nombre,
                                CAST((c.eh_melaza_racion * c.eh_melaza_costo) AS DECIMAL(10,2)) as total_value
                                FROM eh_melaza c 
                                LEFT JOIN equino v ON c.eh_melaza_tagid = v.tagid 
                                ORDER BY c.eh_melaza_fecha DESC";
                      ...
                      if (empty($molassessData)) {
                          ...
                          foreach ($molassessData as $row) {
                              ...
                              echo "<td>" . htmlspecialchars($row['eh_melaza_fecha'] ?? '') . "</td>";                        
                              echo "<td>" . htmlspecialchars($row['animal_nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['eh_melaza_tagid'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['eh_melaza_racion'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['eh_melaza_costo'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['total_value'] ?? '') . "</td>";
                              
                              // Calculate due date and determine status
                              try {
                                  $molassesDate = new DateTime($row['eh_melaza_fecha']);
                                  ...
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_melaza_fecha']);
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-molasses" 
                                          ...
                                          data-tagid="' . htmlspecialchars($row['eh_melaza_tagid'] ?? '') . '"
                                          data-molasses="' . htmlspecialchars($row['eh_melaza_racion'] ?? '') . '"
                                          data-precio="' . htmlspecialchars($row['eh_melaza_costo'] ?? '') . '"
...
$(document).ready(function() {
    ...
    $('#saveNewMolasses').click(function() {
        ...
        Swal.fire({
            ...
            text: `¿Desea registrar la racion de melaza ${formData.racion} kg para el animal con Tag ID ${formData.tagid}?`,
            icon: 'question',
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        // Close the modal
                        var modal = bootstrap.Modal.getInstance(document.getElementById('newMolassesModal'));
                        modal.hide();
                        
                        // Show success message
                        Swal.fire({
                            title: '¡Registro exitoso!',
                            text: 'El registro de racion de melaza ha sido guardado correctamente',
                            icon: 'success',
                            confirmButtonColor: '#28a745'
                        }).then(() => {
    ...
    $('.edit-molasses').click(function() {
                ...
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editMolassesModalLabel">
                            <i class="fas fa-weight me-2"></i>Editar Racion de Melaza
                        </h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                            ...
                            <div class="mb-4">                                
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <input type="text" class="form-control" id="edit_producto" value="${producto}" required>
                                    </span>
                                </div>
                            </div>
                            <div class="mb-4">
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <input type="text" class="form-control" id="edit_etapa" value="${etapa}" required>
                                    </span>
                                </div>
                            </div>                            
                            <div class="mb-4">                                
        ...
        $('#saveEditMolasses').click(function() {
            ...
            Swal.fire({
                ...
                text: `¿Desea actualizar el registro de racion de melaza para el animal con Tag ID ${formData.tagid}?`,
                icon: 'question',
                showCancelButton: true,
                confirmButtonColor: '#28a745',
                cancelButtonColor: '#dc3545',

...
<title>Equino Configuracion Sal</title>
<!-- Link to the Favicon -->
                      ...
                      if (empty($salsData)) {
                          ...
                          foreach ($salsData as $row) {
                              ...
                              echo '        <button class="btn btn-warning btn-sm edit-sal" 
                                              ...
                                              data-sal="' . htmlspecialchars($row['ec_sal_nombre'] ?? '') . '" 
                                              data-etapa="' . htmlspecialchars($row['ec_sal_etapa'] ?? '') . '" 
                                              data-racion="' . htmlspecialchars($row['ec_sal_racion'] ?? '') . '" 
                                              data-costo="' . htmlspecialchars($row['ec_sal_costo'] ?? '') . '" 
                                              data-vigencia="' . htmlspecialchars($row['ec_sal_vigencia'] ?? '') . '"
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['ec_sal_nombre'] ?? '') . "</td>";
                              // Columna 2: Etapa
                              echo "<td>" . htmlspecialchars($row['ec_sal_etapa'] ?? '') . "</td>";
                              
                              // Column 3: Dosis
                              echo "<td>" . htmlspecialchars($row['ec_sal_racion'] ?? 'N/A') . "</td>";
                              
                              // Column 4: Costo
                              echo "<td>" . htmlspecialchars($row['ec_sal_costo'] ?? 'N/A') . "</td>";
                              
                              // Column 5: Vigencia
                              echo "<td>" . htmlspecialchars($row['ec_sal_vigencia'] ?? 'N/A') . "</td>";
                        ...
                        <form id="editSalForm">
                            ...
                            <div class="mb-4">
                                ...
                                <select class="form-select" id="edit_sal" name="sal" required>
                                    ...
                                    $sql_names = "SELECT DISTINCT ec_sal_nombre FROM ec_sal ORDER BY ec_sal_nombre ASC";
                                    $stmt_names = $conn->prepare($sql_names);
                                    $stmt_names->execute();
                                    $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($names as $name_row) {
                                        echo '<option value="' . htmlspecialchars($name_row['ec_sal_nombre']) . '">' . htmlspecialchars($name_row['ec_sal_nombre']) . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>

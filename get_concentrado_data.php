...
try {
    ...
    $query = "
        SELECT 
            c.eh_concentrado_tagid,
            v.nombre as animal_nombre,
            c.eh_concentrado_racion,
            c.eh_concentrado_costo,
            c.eh_concentrado_fecha
        FROM 
            eh_concentrado c
            LEFT JOIN equino v ON c.eh_concentrado_tagid = v.tagid 
        WHERE 
            c.eh_concentrado_fecha IS NOT NULL 
            AND c.eh_concentrado_racion > 0 
            AND c.eh_concentrado_costo > 0
        ORDER BY 
            c.eh_concentrado_fecha ASC
    ";
    ...
    foreach ($data as $row) {
        ...
        $dailyCost = (float)$row['eh_concentrado_racion'] * (float)$row['eh_concentrado_costo'];
        
        // Get date range: start date + validity period
        $startDate = new DateTime($row['eh_concentrado_fecha']);
        ...
        while ($currentDate <= $endDate) {
            ...
            $animalKey = $row['eh_concentrado_tagid'];
            ...
            if (!isset($monthlyData[$monthKey][$animalKey])) {
                $monthlyData[$monthKey][$animalKey] = array(
                    'tagid' => $row['eh_concentrado_tagid'],
    ...
    foreach ($monthlyData as $month => $animals) {
        foreach ($animals as $animalData) {
            $formattedData[] = array(
                ...
                'eh_concentrado_cantidad' => round($animalData['total_cost'], 2),

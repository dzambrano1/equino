<?php
require_once './pdo_conexion.php';  // This creates the PDO connection as $conn
// Now you can use $conn for database queries (PDO connection)
...
// Prepare and execute the query using prepared statements (PDO)
$sql = "SELECT id, tagid, nombre, genero, raza, etapa, grupo, estatus, fecha_nacimiento, image, image2, image3, video 
        FROM equino" . $where_clause . " ORDER BY tagid ASC";
...
// Sexo Counts
$sexoCountsQuery = "SELECT genero, COUNT(*) as count FROM equino" . $where_clause . " GROUP BY genero";

$sexoCountsResult = $conn->prepare($sexoCountsQuery);
if ($sexoCountsResult) {
    $sexoCountsResult->execute($params);
}

$sexoLabels = [];
$sexoCounts = [];

if ($sexoCountsResult && $sexoCountsResult->rowCount() > 0) {
    while ($row = $sexoCountsResult->fetch(PDO::FETCH_ASSOC)) {
...
// Raza Counts
$razaCountsQuery = "SELECT raza, COUNT(*) as count FROM equino" . $where_clause . " GROUP BY raza";

$razaCountsResult = $conn->prepare($razaCountsQuery);
if ($razaCountsResult) {
    $razaCountsResult->execute($params);
}

$razaLabels = [];
$razaCounts = [];

if ($razaCountsResult && $razaCountsResult->rowCount() > 0) {
    while ($row = $razaCountsResult->fetch(PDO::FETCH_ASSOC)) {
        $razaLabels[] = $row['raza'];
...
// Etapa Counts
$etapaCountsQuery = "SELECT etapa, COUNT(*) as count FROM equino" . $where_clause . " GROUP BY etapa";

$etapaCountsResult = $conn->prepare($etapaCountsQuery);
if ($etapaCountsResult) {
    $etapaCountsResult->execute($params);
}

$etapaLabels = [];
$etapaCounts = [];

if ($etapaCountsResult && $etapaCountsResult->rowCount() > 0) {
    while ($row = $etapaCountsResult->fetch(PDO::FETCH_ASSOC)) {
...
// Grupos Counts
$grupoCountsQuery = "SELECT grupo, COUNT(*) as count FROM equino" . $where_clause . " GROUP BY grupo";

$grupoCountsResult = $conn->prepare($grupoCountsQuery);
if ($grupoCountsResult) {
    $grupoCountsResult->execute($params);
}

$grupoLabels = [];
$grupoCounts = [];

if ($grupoCountsResult && $grupoCountsResult->rowCount() > 0) {
    while ($row = $grupoCountsResult->fetch(PDO::FETCH_ASSOC)) {
...
// Estatus Counts
$estatusCountsQuery = "SELECT estatus, COUNT(*) as count FROM equino" . $where_clause . " GROUP BY estatus";

$estatusCountsResult = $conn->prepare($estatusCountsQuery);
if ($estatusCountsResult) {
    $estatusCountsResult->execute($params);
}

$estatusLabels = [];
$estatusCounts = [];

if ($estatusCountsResult && $estatusCountsResult->rowCount() > 0) {
    while ($row = $estatusCountsResult->fetch(PDO::FETCH_ASSOC)) {
...
$tagidsQuery = "SELECT tagid FROM equino" . $where_clause;
$tagidsResult = $conn->prepare($tagidsQuery);
if ($tagidsResult) {
    $tagidsResult->execute($params);
}

$tagids = [];
if ($tagidsResult && $tagidsResult->rowCount() > 0) {
    while ($row = $tagidsResult->fetch(PDO::FETCH_ASSOC)) {
        $tagids[] = $conn->quote($row['tagid']);
    }
}
...
$avgPesoQuery = "
    SELECT 
        DATE_FORMAT(eh_peso_fecha, '%Y-%m') AS peso_month, 
        AVG(eh_peso_animal) AS average_weight 
    FROM 
        eh_peso 
    WHERE 
        eh_peso_tagid IN (" . implode(',', $tagids) . ") 
    GROUP BY 
        peso_month 
    ORDER BY 
        peso_month ASC
...
$avgPesoResult = $conn->prepare($avgPesoQuery);
if ($avgPesoResult) {
    $avgPesoResult->execute();
}
...
if ($avgPesoResult && $avgPesoResult->rowCount() > 0) {
    while ($row = $avgPesoResult->fetch(PDO::FETCH_ASSOC)) {
...
// Fetch average monthly eh_concentrado_racion * eh_concentrado_costo from eh_concentrado table
$avgRacionQuery = "
    SELECT 
        DATE_FORMAT(eh_concentrado_fecha_inicio, '%Y-%m') AS racion_month,
        AVG(eh_concentrado_racion * eh_concentrado_costo) AS average_racion_cost
    FROM 
        eh_concentrado
    GROUP BY 
        racion_month
...
$avgRacionResult = $conn->prepare($avgRacionQuery);
if ($avgRacionResult) {
    $avgRacionResult->execute();
}
...
if ($avgRacionResult && $avgRacionResult->rowCount() > 0) {
    while ($row = $avgRacionResult->fetch(PDO::FETCH_ASSOC)) {
...
// Fetch unique tagids from ec_historicos_tareas_partos
$tagidQuery = "SELECT DISTINCT eh_parto_tagid FROM eh_parto";
$tagidResult = $conn->prepare($tagidQuery);
if ($tagidResult) {
    $tagidResult->execute();
}

$tagids = [];
if ($tagidResult && $tagidResult->rowCount() > 0) {
    while ($row = $tagidResult->fetch(PDO::FETCH_ASSOC)) {
        $tagids[] = $row['eh_parto_tagid'];
    }
}
...
$partoQuery = "
     SELECT 
         DATE_FORMAT(eh_parto_fecha, '%Y-%m') AS yearmonth,
         eh_parto_tagid,
         SUM(eh_parto_numero) AS total_parto
     FROM 
         eh_parto
     GROUP BY 
         DATE_FORMAT(eh_parto_fecha, '%Y-%m'), eh_parto_tagid
     ORDER BY 
         DATE_FORMAT(eh_parto_fecha, '%Y-%m') ASC
";

$partoResult = $conn->prepare($partoQuery);
if ($partoResult) {
    $partoResult->execute();
}
...
if ($partoResult && $partoResult->rowCount() > 0) {
    $rawData = [];
    while ($row = $partoResult->fetch(PDO::FETCH_ASSOC)) {
        $labels[] = $row['yearmonth'];
        $tagid = $row['eh_parto_tagid'];
...
<style>
    ...
    /* Override Bootstrap outline success buttons with ChatGPT green */
    .btn-outline-success {
    ...
    /* Override ALL Bootstrap button colors with ChatGPT theme */
    .btn-primary {
        background-color: #10a37f !important;
        border-color: #10a37f !important;
    }
    ...
    /* Ensure prompt buttons use ChatGPT theme */
    .prompt-btn {
    ...
    .prompt-btn:hover {
        border-color: #10a37f !important;
        background: linear-gradient(135deg, #10a37f, #0d8a6b) !important;
    ...
    .prompt-btn:active {
        background: linear-gradient(135deg, #0d8a6b, #0a6b56) !important;
        border-color: #0a6b56 !important;
    }
    
    /* Override any other Bootstrap button variants */
    .btn-info {
        background-color: #10a37f !important;
        border-color: #10a37f !important;
        color: #ffffff !important;
    ...
    /* Override any button with primary-like behavior */
    button[class*="btn"]:not(.btn-secondary):not(.btn-danger):not(.btn-outline-secondary) {
        background-color: #10a37f !important;
        border-color: #10a37f !important;
        color: #ffffff !important;
    }
    
    button[class*="btn"]:not(.btn-secondary):not(.btn-danger):not(.btn-outline-secondary):hover {
        background-color: #0d8a6b !important;
        border-color: #0a6b56 !important;
    }
    
    /* Override small button variants */
    ...
    .btn-sm.btn-outline-success:focus {
        color: #ffffff !important;
        background-color: #10a37f !important;
        border-color: #10a37f !important;
        box-shadow: 0 0 0 0.2rem rgba(16, 163, 127, 0.25) !important;
    ...
    /* Ensure all small buttons use ChatGPT theme */
    .btn-sm:not(.btn-secondary):not(.btn-danger):not(.btn-outline-secondary) {
        background-color: #10a37f !important;
        border-color: #10a37f !important;
        color: #ffffff !important;
    }
    
    .btn-sm:not(.btn-secondary):not(.btn-danger):not(.btn-outline-secondary):hover {
        background-color: #0d8a6b !important;
        border-color: #0a6b56 !important;
    }
...
<style>
    ...
    /* Chat Bubble Styles - ChatGPT Inspired */
    .chat-bubble {
        position: fixed;
        bottom: 30px;
        right: 30px;
    ...
    .user-message .message-bubble {
        background-color: #10a37f; /* ChatGPT user message color */
        color: #ffffff;
        border-bottom-right-radius: 4px;
    }

    .bot-message .message-bubble {
        background-color: #2d2d2d; /* ChatGPT bot message color */
        color: #ffffff;
...
<div class="container nav-icons-container">
    ...
    <div class="icon-button-container">
        <button onclick="window.location.href='./equino_configuracion.php'" class="icon-button">
...
<div id="chatFullscreen" class="chat-fullscreen hidden">
            ...
            <div class="welcome-message">
                <div class="chat-message bot-message">
                    <div class="message-bubble">
                        <i class="fas fa-user-doctor" style="color: #10a37f;"></i>
    ...
    <div id="floatingPromptMenu" class="floating-prompt-menu hidden">
        ...
        <div class="floating-menu-content">
            ...
            <p style="margin-top: 15px; font-size: 0.9em; color: #aaaaaa; text-align: center;">
...
<script>
    ...
    function registrar(tagid) {
        if (confirm('¿Desea generar un informe completo para este animal?')) {
            ...
            window.location.href = 'equino_report.php?tagid=' + encodeURIComponent(tagid);
        }
    }
</script>
<script>
    ...
    function openUpdateModal(tagid) {
        ...
        console.log('Starting AJAX request to fetch_equino_data.php');
        
        $.ajax({
            url: 'fetch_equino_data.php',
            type: 'GET',
            data: { tagid: tagid },
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="raza" id="updateRaza" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_razas = "SELECT DISTINCT ec_razas_nombre FROM ec_razas ORDER BY ec_razas_nombre";
                                            $result_razas = $conn->prepare($sql_razas);
                                            $result_razas->execute();
                                            while ($row_razas = $result_razas->fetch(PDO::FETCH_ASSOC)) {
                                                echo '<option value="' . htmlspecialchars($row_razas['ec_razas_nombre']) . '">' 
                                                    . htmlspecialchars($row_razas['ec_razas_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="etapa" id="updateEtapa" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_etapa = "SELECT DISTINCT ec_etapas_nombre FROM ec_etapas ORDER BY ec_etapas_nombre";
                                            $result_etapa = $conn->prepare($sql_etapa);
                                            $result_etapa->execute();
                                            while ($row_etapa = $result_etapa->fetch(PDO::FETCH_ASSOC)) {
                                                echo '<option value="' . htmlspecialchars($row_etapa['ec_etapas_nombre']) . '">' 
                                                    . htmlspecialchars($row_etapa['ec_etapas_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="grupo" id="updateGrupo" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_grupos = "SELECT DISTINCT ec_grupos_nombre FROM ec_grupos ORDER BY ec_grupos_nombre";
                                            $result_grupos = $conn->prepare($sql_grupos);
                                            $result_grupos->execute();
                                            while ($row_grupos = $result_grupos->fetch(PDO::FETCH_ASSOC)) {
                                                echo '<option value="' . htmlspecialchars($row_grupos['ec_grupos_nombre']) . '">' 
                                                    . htmlspecialchars($row_grupos['ec_grupos_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating">
                                        <select class="form-select" name="estatus" id="updateEstatus" required>
                                            <option value="">Seleccionar</option>
                                            <?php
                                            $sql_estatus = "SELECT DISTINCT ec_estatus_nombre FROM ec_estatus ORDER BY ec_estatus_nombre";
                                            $result_estatus = $conn->prepare($sql_estatus);
                                            $result_estatus->execute();
                                            while ($row_estatus = $result_estatus->fetch(PDO::FETCH_ASSOC)) {
                                                echo '<option value="' . htmlspecialchars($row_estatus['ec_estatus_nombre']) . '">' 
                                                    . htmlspecialchars($row_estatus['ec_estatus_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
...
async function generateAnimalPDF(tagid) {
    try {
        ...
        const response = await fetch(`equino_report.php?tagid=${encodeURIComponent(tagid)}`, {
            headers: {
        ...
        const menuHTML = `
            ...
            <div class="prompt-menu">
                ...
                <p style="margin-top: 15px; font-size: 0.9em; color: #aaaaaa;">
    ...
    const messageHtml = `
        <div class="chat-message ${messageClass}">
            <div class="message-bubble ${bubbleClass}">
                <i class="${icon}" style="color: ${role === 'bot' ? '#10a37f' : 'inherit'};"></i>

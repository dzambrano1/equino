...
<div class="container scroll-icons-container">
    ...
    <button class="btn btn-outline-secondary mb-3" type="button" onclick="window.location.href='./equino_registrar_vacunacion.php'" data-tooltip="Salud">
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_compra WHERE eh_compra_tagid = ? ORDER BY eh_compra_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_compra ORDER BY eh_compra_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($compraData as $row) {
        $date = new DateTime($row['eh_compra_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_compra_peso']);
        $compraFechaLabels[] = $row['eh_compra_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_compra_precio']);
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_compra_peso']) * floatval($row['eh_compra_precio']);
...
<?php
    ...
    $compraQuery = "SELECT p.*, 
                         ...
                         CAST((p.eh_compra_peso * p.eh_compra_precio) AS DECIMAL(10,2)) as total_value
                  FROM eh_compra p 
                  LEFT JOIN equino v ON p.eh_compra_tagid = v.tagid 
                  ORDER BY p.eh_compra_fecha DESC";
                ...
                .then(data => {
                    if (data.success) {
                        Swal.fire({
                            ...
                            window.location.href = data.redirect || 'equino_registrar_poblacion.php';
                        });
                    } else {
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_deceso WHERE eh_deceso_tagid = ? ORDER BY eh_deceso_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_deceso ORDER BY eh_deceso_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($decesoData as $row) {
        $date = new DateTime($row['eh_deceso_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_deceso_peso']);
        $decesoFechaLabels[] = $row['eh_deceso_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_deceso_peso']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_deceso_peso']) * floatval($row['eh_deceso_precio']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $decesoQuery = "SELECT p.*, 
                         v.nombre as animal_nombre                         
                  FROM eh_deceso p 
                  LEFT JOIN equino v ON p.eh_deceso_tagid = v.tagid 
                  ORDER BY p.eh_deceso_fecha DESC";
...
foreach ($decesoResult as $row) {
    ...
    echo "<td class='text-center'>
            ...
            <button class='btn btn-success btn-sm edit-deceso'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-causa='" . htmlspecialchars($row['eh_deceso_causa']) . "'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_descarte WHERE eh_descarte_tagid = ? ORDER BY eh_descarte_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_descarte ORDER BY eh_descarte_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($descarteData as $row) {
        $date = new DateTime($row['eh_descarte_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_descarte_peso']);
        $descarteFechaLabels[] = $row['eh_descarte_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_descarte_peso']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_descarte_peso']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $descarteQuery = "SELECT p.*, 
                         v.nombre as animal_nombre                         
                  FROM eh_descarte p 
                  LEFT JOIN equino v ON p.eh_descarte_tagid = v.tagid 
                  ORDER BY p.eh_descarte_fecha DESC";
...
foreach ($descarteResult as $row) {
    ...
    echo "<td class='text-center'>
            <button class='btn btn-success btn-sm edit-descarte'
                    data-id='" . htmlspecialchars($row['id']) . "'
                    data-causa='" . htmlspecialchars($row['eh_descarte_peso']) . "'
...
try {
    if (isset($_GET['search']) && !empty($_GET['search'])) {
        $stmt = $conn->prepare("SELECT * FROM eh_venta WHERE eh_venta_tagid = ? ORDER BY eh_venta_fecha ASC");
        $stmt->execute([$_GET['search']]);
    } else {
        $stmt = $conn->prepare("SELECT * FROM eh_venta ORDER BY eh_venta_fecha ASC");
        $stmt->execute();
    }
    ...
    foreach ($ventaData as $row) {
        $date = new DateTime($row['eh_venta_fecha']);
        $monthKey = $date->format('Y-m');

        if (!isset($monthlyWeights[$monthKey])) {
            $monthlyWeights[$monthKey] = [];
        }
        $monthlyWeights[$monthKey][] = floatval($row['eh_venta_peso']);
        $ventaFechaLabels[] = $row['eh_venta_fecha'];
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $monthlyPrices[$monthKey]['sum'] += floatval($row['eh_venta_peso']);
        $monthlyPrices[$monthKey]['count']++;
    }
    ...
    while ($row = $stmt->fetch()) {
        ...
        $totalValue = floatval($row['eh_venta_peso']) * floatval($row['eh_venta_precio']);
        $monthlyValues[$monthKey]['sum'] += $totalValue;
        $monthlyValues[$monthKey]['count']++;
    }
...
<?php
    ...
    $ventaQuery = "SELECT p.*, 
                         v.nombre as animal_nombre,
                         CAST((p.eh_venta_peso * p.eh_venta_precio) AS DECIMAL(10,2)) as total_value
                  FROM eh_venta p 
                  LEFT JOIN equino v ON p.eh_venta_tagid = v.tagid 
                  ORDER BY p.eh_venta_fecha DESC";

...
<title>equino Configuracion Etapas</title>
<!-- Link to the Favicon -->
...
<div class="modal fade" id="newEntryModal" tabindex="-1" aria-labelledby="newEntryModalLabel">
  ...
  <!-- DataTable for ec_etapas records -->
...
<div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="etapasTable" class="table table-striped table-bordered">
              ...
              <tbody>
                  <?php
                      $etapasQuery = "SELECT * FROM ec_etapas";
                      ...
                      if (empty($etapassData)) {
                          ...
                          foreach ($etapassData as $row) {
                              ...
                              echo '        <button class="btn btn-danger btn-sm delete-etapas" 
                                              ...
                                              data-etapas="' . htmlspecialchars($row['ec_etapas_nombre'] ?? '') . '"
                              ...
                              // Column 1: Vacuna
                              echo "<td>" . htmlspecialchars($row['ec_etapas_nombre'] ?? '') . "</td>";
                                ...
                                <div class="input-group">
                                    <span class="input-group-text">
                                        ...
                                        <select class="form-select" id="edit_etapas" name="etapas" required>
                                            ...
                                            $sql_names = "SELECT DISTINCT ec_etapas_nombre FROM ec_etapas ORDER BY ec_etapas_nombre ASC";
                                            $stmt_names = $conn->prepare($sql_names);
                                            $stmt_names->execute();
                                            $names = $stmt_names->fetchAll(PDO::FETCH_ASSOC);
                                            foreach ($names as $name_row) {
                                                echo '<option value="' . htmlspecialchars($name_row['ec_etapas_nombre']) . '">' . htmlspecialchars($name_row['ec_etapas_nombre']) . '</option>';
                                            }
                                            ?>
                                        </select>
                                    </span>                                    

...
<title>Equino - Registro de Vacunación Cbr</title>
<!-- Link to the Favicon -->
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT ec_cbr_vacuna FROM ec_cbr ORDER BY ec_cbr_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['ec_cbr_vacuna']) . '">' . htmlspecialchars($vacuna_row['ec_cbr_vacuna']) . '</option>';
                                        }
              ...
              <tbody>
                  ...
                  try {
                      $cbrQuery = "SELECT 
                                          ...
                                          CASE WHEN va.eh_cbr_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_cbr_history
                                      FROM 
                                          equino v
                                      LEFT JOIN 
                                          -- Join with all cbr records per tagid
                                          eh_cbr va ON v.tagid = va.eh_cbr_tagid 
                                      ORDER BY 
                                          va.eh_cbr_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($cbrsData)) {
                          ...
                          try {
                              $configQuery = "SELECT e_vencimiento_cbr FROM e_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['e_vencimiento_cbr'])) {
                                  $vigencia = intval($row_config['e_vencimiento_cbr']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($cbrsData as $row) {
                              ...
                              if ($row['in_cbr_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-cbr" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['eh_cbr_producto'] ?? '') . '" 
                              ...
                              echo "<td>" . htmlspecialchars($row['eh_cbr_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_cbr_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['eh_cbr_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_cbr_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_cbr_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['eh_cbr_fecha'])) {
                                          $cbrDate = new DateTime($row['eh_cbr_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_cbr_fecha']);
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['ec_cbr_vacuna']) . '">' . htmlspecialchars($vacuna_row['ec_cbr_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());

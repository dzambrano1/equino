...
try {
    ...
    $sql = "SELECT 
                DATE_FORMAT(eh_parto_fecha, '%Y-%m') as month,
                COUNT(*) as birth_count,
                AVG(eh_parto_numero) as avg_birth_number,
                MAX(eh_parto_numero) as max_birth_number,
                COUNT(*) as record_count
            FROM eh_parto 
            WHERE eh_parto_fecha IS NOT NULL 
            AND eh_parto_numero > 0
            GROUP BY DATE_FORMAT(eh_parto_fecha, '%Y-%m')
            ORDER BY DATE_FORMAT(eh_parto_fecha, '%Y-%m') ASC";

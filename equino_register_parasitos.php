...
<title>Equino - Registro de Vacunación Parasitos</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT ec_parasitos_vacuna FROM ec_parasitos ORDER BY ec_parasitos_vacuna ASC";
                                        ...
                                        foreach ($vacunas as $vacuna_row) {
                                            echo '<option value="' . htmlspecialchars($vacuna_row['ec_parasitos_vacuna']) . '">' . htmlspecialchars($vacuna_row['ec_parasitos_vacuna']) . '</option>';
                                        }
  ...
  <!-- DataTable for eh_parasitos records -->
              ...
              <tbody>
                  ...
                  try {
                      $parasitosQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.eh_parasitos_fecha,
                                          va.id AS parasitos_record_id, -- Alias parasitos record ID
                                          va.eh_parasitos_producto,
                                          va.eh_parasitos_dosis,
                                          va.eh_parasitos_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.eh_parasitos_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_parasitos_history
                                      FROM 
                                          equino v
                                      LEFT JOIN 
                                          -- Join with all parasitos records per tagid
                                          eh_parasitos va ON v.tagid = va.eh_parasitos_tagid 
                                      ORDER BY 
                                          va.eh_parasitos_fecha DESC, v.tagid ASC;";
                          ...
                          foreach ($parasitossData as $row) {
                              ...
                              if ($row['in_parasitos_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-parasitos" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['eh_parasitos_producto'] ?? '') . '" 
                                                  data-dosis="' . htmlspecialchars($row['eh_parasitos_dosis'] ?? '') . '" 
                                                  data-costo="' . htmlspecialchars($row['eh_parasitos_costo'] ?? '') . '" 
                                                  data-fecha="' . htmlspecialchars($row['eh_parasitos_fecha'] ?? '') . '"
                                                  title="Editar Vacuna">
                              ...
                              echo "<td>" . htmlspecialchars($row['eh_parasitos_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_parasitos_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['eh_parasitos_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_parasitos_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_parasitos_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['eh_parasitos_fecha'])) {
                                          $parasitosDate = new DateTime($row['eh_parasitos_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_parasitos_fecha']);
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="vacunaList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $vacunas = $stmt_vacunas->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($vacunas as $vacuna_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action vacuna-option" data-value="' . htmlspecialchars($vacuna_row['ec_parasitos_vacuna']) . '">' . htmlspecialchars($vacuna_row['ec_parasitos_vacuna']) . '</button>';
                                    }
                                } catch (PDOException $e) {
                                    error_log("Error fetching vacunas: " . $e->getMessage());

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'update' && isset($_POST['tagid'], $_POST['peso'], $_POST['precio'], $_POST['fecha'])) {
        try {
            // Update equino table with descarte information and set status to Descarte
            $stmt = $conn->prepare("UPDATE equino SET 
                descarte_fecha = ?, 
                descarte_peso = ?,
                descarte_precio = ?,
                estatus = 'Descarte'
            ...
            if ($stmt->rowCount() > 0) {
                $response = array(
                    ...
                    'redirect' => 'equino_register_descarte.php'
                );
            } else {
                $response = array(
                    'success' => false,
        ...
        try {
            // Clear descarte information from equino table and set status back to Activo
            $stmt = $conn->prepare("UPDATE equino SET 
                descarte_fecha = NULL, 
                descarte_peso = NULL,

...
<div class="container text-center">
  ...
  <!-- DataTable for eh_garrapatas records -->
              ...
              <tbody>
                  ...
                  try {
                      $garrapatasQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.eh_garrapatas_fecha,
                                          va.id AS garrapatas_record_id, -- Alias garrapatas record ID
                                          va.eh_garrapatas_producto,
                                          va.eh_garrapatas_dosis,
                                          va.eh_garrapatas_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.eh_garrapatas_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_garrapatas_history
                                      FROM 
                                          equino v
                                      LEFT JOIN 
                                          -- Join with all garrapatas records per tagid
                                          eh_garrapatas va ON v.tagid = va.eh_garrapatas_tagid 
                                      ORDER BY 
                                          va.eh_garrapatas_fecha DESC, v.tagid ASC;";
                          ...
                          foreach ($garrapatassData as $row) {
                              ...
                              if ($row['in_garrapatas_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-garrapatas" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['eh_garrapatas_producto'] ?? '') . '" 
                              ...
                              echo "<td>" . htmlspecialchars($row['eh_garrapatas_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_garrapatas_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['eh_garrapatas_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_garrapatas_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_garrapatas_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['eh_garrapatas_fecha'])) {
                                          $garrapatasDate = new DateTime($row['eh_garrapatas_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_garrapatas_fecha']);

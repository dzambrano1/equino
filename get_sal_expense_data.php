...
try {
    ...
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            $startDate = new DateTime($row['eh_sal_fecha_inicio']);
            $endDate = new DateTime($row['eh_sal_fecha_fin']);
            ...
            // eh_sal_racion is a daily figure, so we multiply by the number of days
            $totalExpense = (float)$row['eh_sal_racion'] * (float)$row['eh_sal_costo'] * $totalDays;
            ...
            while ($currentDate <= $endDate) {
                ...
                $monthlyData[$monthKey]['total_quantity'] += (float)$row['eh_sal_racion'];
                $monthlyData[$monthKey]['total_cost_sum'] += (float)$row['eh_sal_costo'];

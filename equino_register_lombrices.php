...
<title>Equino Register Lombrices</title>
<!-- Link to the Favicon -->
...
<div class="container" id="nav-buttons">
    <div class="container nav-icons-container" id="nav-buttons">
        ...
        <button onclick="window.location.href='./equino_historial.php'" class="icon-button" data-tooltip="Registrar Ganado">
...
<div class="container scroll-icons-container">
    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-poblacion-equino" data-tooltip="Poblacion">
        <img src="./images/vaca.png" alt="Alimentacion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-produccion-equino" data-tooltip="Produccion">
        <img src="./images/Bascula.png" alt="Produccion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-alimentacion-equino" data-tooltip="Alimentacion">
        <img src="./images/bolso.png" alt="Alimentacion" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-salud-equino" data-tooltip="Salud">
        <img src="./images/vacunacion.png" alt="Salud" class="nav-icon">
    </button>
       
    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-reproduccion-equino" data-tooltip="Reproducion">
        <img src="./images/matriz.png" alt="Razas" class="nav-icon">
    </button>

    <button class="btn btn-outline-secondary mb-3" type="button" data-bs-toggle="collapse" data-bs-target="#section-inventario-otros-equino" data-tooltip="Otros">
...
<div class="container text-center">
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_vacuna" name="vacuna" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_vacunas = "SELECT DISTINCT vacunas_nombre_comercial FROM e_vacunas ORDER BY vacunas_nombre_comercial ASC";
  ...
  <!-- DataTable for eh_lombrices records -->
  
  <div class="container table-section" style="display: block;">
      <div class="table-responsive">
          <table id="eh_lombricesTable" class="table table-striped table-bordered">
              <thead>
                  <tr>
              ...
              <tbody>
                  <?php
                  try {
                      // Query to get all eh_lombrices records with animal name
                      $lombricesQuery = "SELECT c.*, v.nombre as animal_nombre,
                                CAST((c.eh_lombrices_dosis * c.eh_lombrices_costo) AS DECIMAL(10,2)) as total_value
                                FROM eh_lombrices c 
                                LEFT JOIN equino v ON c.eh_lombrices_tagid = v.tagid 
                                ORDER BY c.eh_lombrices_fecha DESC";
                      ...
                      if (empty($lombricesData)) {
                          ...
                          try {
                              $configQuery = "SELECT e_vencimiento_lombrices FROM e_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              
                              // Explicitly use PDO fetch method
                              $row = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row && isset($row['e_vencimiento_lombrices'])) {
                                  $vigencia = intval($row['e_vencimiento_lombrices']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($lombricesData as $row) {
                              ...
                              try {
                                  $lombricesDate = new DateTime($row['eh_lombrices_fecha']);
                                  ...
                                  error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_lombrices_fecha']);
                              ...
                              echo '<td class="text-center">
                                  <div class="btn-group" role="group">
                                      <button class="btn btn-warning btn-sm edit-lombrices" 
                                          ...
                                          data-tagid="' . htmlspecialchars($row['eh_lombrices_tagid'] ?? '') . '"
                                          data-vacuna="' . htmlspecialchars($row['eh_lombrices_producto'] ?? '') . '"

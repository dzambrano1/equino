...
try {
    ...
    $salQuery = "SELECT 
                        ...
                        AND eh_sal_fecha_fin IS NOT NULL 
                        AND eh_sal_racion IS NOT NULL 
                        AND eh_sal_costo IS NOT NULL
                        ORDER BY eh_sal_fecha_inicio ASC";
    ...
    if (empty($salData)) {
        ...
        $salQuery = "SELECT 
                                eh_sal_fecha,
                                eh_sal_fecha as eh_sal_fecha_inicio,
                                eh_sal_fecha as eh_sal_fecha_fin,
                                eh_sal_racion,
                                eh_sal_costo,
                                eh_sal_tagid
                            FROM eh_sal 
                            WHERE eh_sal_fecha IS NOT NULL 
                            AND eh_sal_racion IS NOT NULL 
                            AND eh_sal_costo IS NOT NULL
                            ORDER BY eh_sal_fecha ASC";
    ...
    foreach ($salData as $sal) {
        $startDate = new DateTime($sal['eh_sal_fecha_inicio']);
        $endDate = new DateTime($sal['eh_sal_fecha_fin']);
        
        // Calculate total expense value
        $totalDays = $endDate->diff($startDate)->days + 1;
        $totalValue = $sal['eh_sal_racion'] * $sal['eh_sal_costo'] * $totalDays;
        $dailyValue = $totalValue / $totalDays;
        
        error_log("Processing sal: Start=" . $sal['eh_sal_fecha_inicio'] . 
                 ", End=" . $sal['eh_sal_fecha_fin'] . 
                 ", Days=" . $totalDays . 
...
<title>equino - Registro de Sal</title>
<!-- Link to the Favicon -->
                        ...
                        <div class="input-group">
                            <span class="input-group-text">
                                ...
                                <select class="form-select" id="new_alimento" name="alimento" required>
                                    ...
                                    <?php
                                    try {
                                        $sql_alimentos = "SELECT DISTINCT ec_sal_nombre FROM ec_sal ORDER BY ec_sal_nombre ASC";
                                        ...
                                        foreach ($alimentos as $alimento_row) {
                                            echo '<option value="' . htmlspecialchars($alimento_row['ec_sal_nombre']) . '">' . htmlspecialchars($alimento_row['ec_sal_nombre']) . '</option>';
                                        }
                                    } catch (PDOException $e) {
            ...
            try {
                      // Query to get all Animals and ALL their Sal records (if any)
                        $salQuery = "
                            SELECT
                                ...
                                c.eh_sal_fecha_inicio,
                                c.eh_sal_fecha_fin,
                                c.eh_sal_tagid,         -- Matches equino_tagid if sal exists
                                c.eh_sal_etapa,
                                c.eh_sal_producto,
                                c.eh_sal_racion,
                                c.eh_sal_costo,
                                -- Calculate total_value only if c.id is not null
                                CASE WHEN c.id IS NOT NULL THEN CAST((c.eh_sal_racion * c.eh_sal_costo) AS DECIMAL(10,2)) ELSE NULL END as total_value
                            FROM
                                equino v
                            LEFT JOIN
                                eh_sal c ON v.tagid = c.eh_sal_tagid -- Join ALL matching sal records
                            ORDER BY
                      ...
                      if (empty($salData)) {
                          ...
                          foreach ($salData as $row) {
                              ...
                              $salFecha = $row['eh_sal_fecha_inicio'] ?? null;
                              $salFechaInicio = $row['eh_sal_fecha_inicio'] ?? null;
                              $salFechaFin = $row['eh_sal_fecha_fin'] ?? null;
                              ...
                              if ($hasSal) {
                                  ...
                                  echo '        <button class="btn btn-warning btn-sm edit-sal" 
                                                  ...
                                                  data-debug="DB: '.htmlspecialchars($row['eh_sal_producto'] ?? 'NULL').'">
                              ...
                              echo "<td>" . ($hasSal ? htmlspecialchars($row['eh_sal_producto'] ?? '') : 'N/A') . "</td>";
                              // Column 7: Racion (or N/A)
                              echo "<td>" . ($hasSal ? htmlspecialchars($row['eh_sal_racion'] ?? '') : 'N/A') . "</td>";
                              // Column 8: Costo (or N/A)
                              echo "<td>" . ($hasSal ? htmlspecialchars($row['eh_sal_costo'] ?? '') : 'N/A') . "</td>";
                        ...
                        <div class="mb-3">
                            ...
                            <div class="list-group" id="productList" style="max-height: 300px; overflow-y: auto;">
                                ...
                                try {
                                    ...
                                    $alimentos = $stmt_alimentos->fetchAll(PDO::FETCH_ASSOC);
                                    foreach ($alimentos as $alimento_row) {
                                        echo '<button type="button" class="list-group-item list-group-item-action product-option" data-value="' . htmlspecialchars($alimento_row['ec_sal_nombre']) . '">' . htmlspecialchars($alimento_row['ec_sal_nombre']) . '</button>';
                                    }
                                } catch (PDOException $e) {

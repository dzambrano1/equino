...
<title>Equino - Registro de Vacunación Carbunco</title>
<!-- Link to the Favicon -->
...
<div class="container text-center">
  ...
  <!-- DataTable for eh_carbunco records -->
              ...
              <tbody>
                  ...
                  try {
                      $carbuncoQuery = "SELECT 
                                          ...
                                          v.tagid,
                                          v.nombre,
                                          va.eh_carbunco_fecha,
                                          va.id AS carbunco_record_id, -- Alias carbunco record ID
                                          va.eh_carbunco_producto,
                                          va.eh_carbunco_dosis,
                                          va.eh_carbunco_costo,
                                          -- Flag to easily check if there's a history record
                                          CASE WHEN va.eh_carbunco_tagid IS NOT NULL THEN 1 ELSE 0 END AS in_carbunco_history
                                      FROM 
                                          equino v
                                      LEFT JOIN 
                                          -- Join with all carbunco records per tagid
                                          eh_carbunco va ON v.tagid = va.eh_carbunco_tagid 
                                      ORDER BY 
                                          va.eh_carbunco_fecha DESC, v.tagid ASC;";
                      ...
                      if (empty($carbuncosData)) {
                          ...
                          try {
                              $configQuery = "SELECT e_vencimiento_carbunco FROM e_vencimiento LIMIT 1";
                              $configStmt = $conn->prepare($configQuery);
                              $configStmt->execute();
                              $row_config = $configStmt->fetch(PDO::FETCH_ASSOC);
                              if ($row_config && isset($row_config['e_vencimiento_carbunco'])) {
                                  $vigencia = intval($row_config['e_vencimiento_carbunco']);
                              }
                          } catch (PDOException $e) {
                          ...
                          foreach ($carbuncosData as $row) {
                              ...
                              if ($row['in_carbunco_history'] == 1) {
                                  echo '        <button class="btn btn-warning btn-sm edit-carbunco" 
                                                  ...
                                                  data-vacuna="' . htmlspecialchars($row['eh_carbunco_producto'] ?? '') . '" 
                              ...
                              echo "<td>" . htmlspecialchars($row['eh_carbunco_fecha'] ?? '') . "</td>";
                              echo "<td>" . htmlspecialchars($row['nombre'] ?? 'N/A') . "</td>";
                              echo "<td>" . htmlspecialchars($row['tagid'] ?? '') . "</td>";

                              if ($row['in_carbunco_history'] == 1) {
                                  // Animal has vaccination history
                                  echo "<td>" . htmlspecialchars($row['eh_carbunco_producto'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_carbunco_dosis'] ?? 'N/A') . "</td>";
                                  echo "<td>" . htmlspecialchars($row['eh_carbunco_costo'] ?? 'N/A') . "</td>";

                                  // Calculate due date and determine status
                                  try {
                                      if (!empty($row['eh_carbunco_fecha'])) {
                                          $carbuncoDate = new DateTime($row['eh_carbunco_fecha']);
                                      ...
                                      error_log("Date error: " . $e->getMessage() . " for date: " . $row['eh_carbunco_fecha']);

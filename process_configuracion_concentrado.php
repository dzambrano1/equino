...
try {
    ...
    switch ($action) {
        case 'insert':
            ...
            $sql = "INSERT INTO ec_concentrado (ec_concentrado_nombre, ec_concentrado_etapa, ec_concentrado_costo, ec_concentrado_vigencia) 
        ...
        case 'update':
            ...
            // Update existing record in ec_concentrado
            $sql = "UPDATE ec_concentrado 
                    SET ec_concentrado_nombre = :concentrado, 
                        ec_concentrado_etapa = :etapa,
                        ec_concentrado_costo = :costo, 
                        ec_concentrado_vigencia = :vigencia 
                    WHERE id = :id";
            ...
            if (!$stmt->execute()) {
                throw new Exception("Error al actualizar el registro en ec_concentrado: " . implode(", ", $stmt->errorInfo()));
            }
            
            // Update related records in eh_concentrado table
            $sql_vh = "UPDATE eh_concentrado 
                       SET eh_concentrado_etapa = :etapa 
                       WHERE eh_concentrado_producto = :concentrado";
            ...
            if (!$stmt_vh->execute()) {
                error_log("Warning: Could not update eh_concentrado records: " . implode(", ", $stmt_vh->errorInfo()));
            }
            
            // Update related records in equino table for animals using this concentrado
            $sql_equino = "UPDATE equino 
                           SET etapa = :etapa 
                           WHERE tagid IN (
                               SELECT DISTINCT eh_concentrado_tagid 
                               FROM eh_concentrado 
                               WHERE eh_concentrado_producto = :concentrado
                           )";
            
            $stmt_equino = $conn->prepare($sql_equino);
            $stmt_equino->bindValue(':etapa', $etapa, PDO::PARAM_STR);
            $stmt_equino->bindValue(':concentrado', $concentrado, PDO::PARAM_STR);
            
            if (!$stmt_equino->execute()) {
                error_log("Warning: Could not update equino records: " . implode(", ", $stmt_equino->errorInfo()));
            }
        ...
        case 'delete':
            ...
            // Delete record
            $sql = "DELETE FROM ec_concentrado WHERE id = :id";

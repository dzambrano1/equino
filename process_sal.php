...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'insert' && isset($_POST['tagid'], $_POST['racion'], $_POST['producto'], $_POST['etapa'], $_POST['costo'], $_POST['fecha_inicio'], $_POST['fecha_fin'])) {
        try {
            ...
            // Insert into eh_sal table
            $stmt = $conn->prepare("INSERT INTO eh_sal (eh_sal_tagid, eh_sal_racion, eh_sal_producto, eh_sal_etapa, eh_sal_costo, eh_sal_fecha_inicio, eh_sal_fecha_fin) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $_POST['tagid'],
                $_POST['racion'],
                $_POST['producto'],
            ...
            $response = array(
                'success' => true,
                'message' => 'Registro agregado correctamente en eh_sal y equino',
                'redirect' => 'equino_register_sal.php'
            );
        ...
        try {
            ...
            // First, update the eh_sal table
            $stmt = $conn->prepare("UPDATE eh_sal SET eh_sal_racion = ?, eh_sal_producto = ?, eh_sal_etapa = ?, eh_sal_costo = ?, eh_sal_fecha_inicio = ?, eh_sal_fecha_fin = ? WHERE id = ?");
            $stmt->execute([
                $_POST['racion'],
                $_POST['producto'],
                $_POST['etapa'],
            ...
            $response = array(
                'success' => true,
                'message' => 'Registro actualizado correctamente en eh_sal y equino',
                'redirect' => 'equino_register_sal.php'
            );
